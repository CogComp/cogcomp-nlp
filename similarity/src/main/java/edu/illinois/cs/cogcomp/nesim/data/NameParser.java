...
public class NameParser {
	...
	private String extraName ; 
		// if there are more than one names, the last one is chosen to be parsed. Rest are put in this variable.
	...
	/**
	 * Constructor to initialize a NameParser object. Used in scoring strings of type PER.
	 * 
	 * @param honorificFileName	File name to obtain list of name strings
	 * @param nicknameFileName	File name to obtain list of nickname strings
	 * @param nickThres	Threshold at which a name is considered a nickname
	 * @throws IOException
	 */
	...
	/**
	 * Parses a given string of type PER, identifying the parts of the name - first name, last name, 
	 * honorific, title, etc.
	 * 
	 * @param name	String to be parsed using type PER
	 */
	public void parsing(String name) {
		...
		int reversedNamePosn = -1; 
			// -1 indicates not reversed, 0 or more indicates index where the last name ends
	...
	/**
	 * Helper function to parse a string. Identifies honorifics, first/last names, and suffixes.
	 * 
	 * @param tokens	Array of all words in the string
	 * @param index	Array index at which the first or last name begins
	 * @param lastNameLen	Length of the last name
	 * @param swapNameOrder Whether the first/last name order is swapped
	 */
	...
	/**
	 * Compares two strings of type PER that are converted into NameParser objects. Primary function
	 * called by EntityComparison.
	 * 
	 * @param nameParser	The other string of type PER to be compared with
	...
	public float compare(NameParser nameParser) {
		...
		float score = 0.0f ;
		// Check to see if all the conditions are satisfied; return false if there is any condition failed.
		// 0. Matching special cases
		matchType = specialCasesMatch(nameParser) ;
		if(matchType == 0) {
			if (lastNameMatch(nameParser.getLastName())) {
				// 1. Match last name AND 2. Match first name (with or without nicknames)
				firstNameMatchType = firstNameMatch(nameParser.getFirstName()) ;
				switch(firstNameMatchType) {
					case 1 : score = 1.0f ; break ; // both last and first names match exactly
					case 2 : score = 0.75f ; break ; // last name matches and first names match as nicknames of a common name
					default : score = 0.0f ; // last name matches, first name exists and doesn't match
				}
			}
			return score ;
		} else if(matchType == 3) {
			...
			if (lastNameMatch(nameParser.getLastName())) {
				...
				switch(firstNameMatchType) {
					case 1 : score = 1.0f ; break ; // both last and first names match exactly
					default: score = 0.9f ; // last name matches, first name exists (subsumption), but not recognized as one
					//case 2 : score = 0.75f ; break ; // last name matches and first names match as nicknames of a common name
				}
			}
			return score ;
		...
		if (!honorificMatching(nameParser.getHonorific()))
			score = 0.0f; // As of now honorifics are not handled. So, instead of returning here, just continue
		switch(matchType) {
			case 1: score = 0.93f ; break ; // match first name
			case 2: score = 0.95f ; break ; // match last name
			case 4: score = 0.75f ; break ; // match first names that are both nicknames
			case 3: score = 0.90f ; break ; // name subsumed
		}
		return score ;
	}
	...
	private int specialCasesMatch(NameParser nameParser) {
		...
		if( this.firstName.equals(this.lastName) ||
				nameParser.getFirstName().equals(nameParser.getLastName()) ) {
	 ...
	 * 
	 * @param thisName	First of the two compared strings
	 * @param thatName	Second of the two compared strings
	 * @return
	 */
	private float lastNameMatchScore(String thisName, String thatName) {
		...
		if(score < TYPOTHRES) {
			...
			if( (thisLastNames.size() >= thatLastNames.size() && isSubsumed(thatLastNames, thisLastNames)) ||
				...
				score = 0.9f ; // If name match fails, check for subsumption and allow token subsets
		}
		return score ;
	}
	 ...
	 * 
	 * @param lastName	Last name parsed from the string
	 * @return	A boolean value (true/false) determining whether the last name is valid
	 */
	...
	/**
	 * Checks if the two given names are compatible first names. Returns 0 if the names are not compatible, 
	 * > 0 if names are compatible and above threshold, and < 0: if names are both nicknames, but of a
	 * common name, and pass threshold
	 * 
	 * @param thisName	First of the two first name strings
	 * @param thatName	Second of the two first name strings
	 * @return	A float value determining whether the two first names are compatible
	 */
	...
	/**
	 * Checks for matching first names in the ArrayList. Returns 0 if no match, 1 if
	 * names are compatible, and 2 if both are nicknames for a common name.
	 * 
	 * @param firstName	First name parsed from the string
	 ...
	 * 
	 * @param honorific	String to be checked for honorific
	 * @return	A boolean value (true/false) determining whether the string is a honorific
	 */
