package edu.illinois.cs.cogcomp.entitySimilarity.utils;
...
public class SpellingCorrector {
	...
	/**
	 * Generates a hash table for all entities present in "DbpediaPerson.txt" and "GeoPlaces.txt".
	 * The hashing function used is the soundex function.
	 * Also, generates generates two list containing all the above entities.
	 * The first list is sorted by the first OFFSET characters while the second list is sorted by the last 
	 * OFFSET characters. 
	 * These lists are used for fast retrieval in the function @correctSpelling.
	 */
	@SuppressWarnings("unchecked")
	private void generateHash() {
		try {
			InputStream in = SpellingCorrector.class.getResourceAsStream("/edu/illinois/cs/cogcomp/nesim/files/DbpediaPersons.txt");
			...
			while ((line = reader.readLine()) != null) {
				...
				int index = (code.charAt(0)-'A')*1000+(code.charAt(1)-'0')*100+(code.charAt(2)-'0')*10+(code.charAt(3)-'0');
				hash[index].add(line);
				sort1.add(line);
				sort2.add(line);
	...
	/**
	 * Corrects spelling of @word i.e. returns a set of possible corrections for @word.
	 * For e.g. ["packistan"] => [{"pakestan", "pakistan"}]
	 * It currently returns all entities that are 1 edit distance away from @word + 
	 * 						all entities that are 2 edit distance away from @word and whose soundex code matches with @word
	 * The edit distance used is DameraoLevenstein.
	 * If the list is empty, it returns the entity that is closest from @word.
	 * This spelling correction currently handles entities of types PER and LOC.
	 * It uses the list of entities present in the files "DbpediaPersons.txt" and "GeoPlaces.txt" as a Knowledge Base.
	 * @param word
	 * @return
	 * @author ashwink
	 */
