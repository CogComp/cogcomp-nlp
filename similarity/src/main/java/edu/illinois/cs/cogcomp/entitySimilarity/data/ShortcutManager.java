...
import edu.illinois.cs.cogcomp.entitySimilarity.utils.LSHUtils;
...
public class ShortcutManager {
	...
	/**
	 * Constructor to initialize a ShortcutManager object. Used to score strings of type LOC that
	 * may be shortcuts.
	 * 
	 * @param shortcutFileName	File name to obtain list of shortcut strings
	 * @throws IOException
	 */
	...
	/**
	 * Reads the given file and stores the shortcut and long form names in a map.
	 * 
	 * @return	A Map containing all possible shortcuts for a particular long form string
	 */
	...
	/**
	 * Primary scoring function invoked by EntityComparison. Used to score strings of type LOC
	 * where one string may be a shortcut of the other string.
	 * 
	 * @param name1	First of the two strings to be scored
	 * @param name2	Second of the two strings to be scored
		...
		if (isShortcutPair(shortName, longName))
			...
			try {
				...
				if (shouldIgnore) {
					...
					if (newScore < 0.85f) // adjust threshold as necessary for optimal results
						score = 0.0f;
					else
						score = 1.0f;
					if (diffScore <= 1.0f) // allow for up to 1 misspelled letter
						score = 1.0f;
					return score;
				}
...
//		String [] shortened = {name1, name2};
	...
	/**
	 * Returns true if the shorter string is a valid shortcut for the longer string.
	 * 
	 * @param shortName	The shorter of the two strings being compared
	 * @param longName	The longer of the two strings being compared
	 * @return	A boolean value (true/false) determining whether the shorter string is a shortcut
	 ...
	 * 
	 * @param shortcut	The string that may be a shortcut
	 ...
	 * 
	 * @param longform	The string that may be a long form
