 ...
 */
package edu.cs.cogcomp.ner.LbjTagger;

import edu.cs.cogcomp.ner.ExpressiveFeatures.BrownClusters;
import edu.cs.cogcomp.ner.ExpressiveFeatures.GazetteersFactory;
import edu.cs.cogcomp.ner.ExpressiveFeatures.TitleTextNormalizer;
import edu.cs.cogcomp.ner.ExpressiveFeatures.WordEmbeddings;
import edu.cs.cogcomp.ner.LbjFeatures.NETaggerLevel1;
import edu.cs.cogcomp.ner.LbjFeatures.NETaggerLevel2;
import edu.cs.cogcomp.ner.config.NerBaseConfigurator;
import edu.illinois.cs.cogcomp.core.constants.Language;
import edu.illinois.cs.cogcomp.core.datastructures.ViewNames;
import edu.illinois.cs.cogcomp.core.utilities.configuration.ResourceManager;
import edu.illinois.cs.cogcomp.lbjava.io.IOUtilities;
import edu.cs.cogcomp.ner.config.NerOntonotesConfigurator;
...
public class Parameters {
            ...
            if (rm.containsKey("WordEmbeddings") && rm.getString("WordEmbeddings").equals("1")) {
                ...
                Vector<Double> normalizationConstantsForEmbeddings =
                        getDoubleVector(rm.getString("normalizationConstantsForEmbeddings").split(
                                "\\s+")); // list
                Vector<WordEmbeddings.NormalizationMethod> normalizationMethodsForEmbeddings =
    ...
    private static Vector<WordEmbeddings.NormalizationMethod> getNMVector(String[] arr) {
        Vector<WordEmbeddings.NormalizationMethod> v = new Vector<>();
        for (String s : arr) {
            v.add(WordEmbeddings.NormalizationMethod.valueOf(s));
        }
        return v;
    }
    // ===============================================================
