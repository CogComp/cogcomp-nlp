...
/**
 * A configuration helper to allow centralization of config options in dependencies with clear
 * default settings (only values that override defaults need to be specified).
 *
 * NOTE: The parameters for Penn Treebank, Nombank, and Propbank are set to directories on CCG
 * servers, so must be overridden on other machines for uses of SRL that require these corpora
 * (presumably, for training only).
 *
...
public class SrlConfigurator extends AnnotatorConfigurator {
    ...
    /**
     * The ILP solver to use for the joint inference Options are: Gurobi, OJAlgo
     */
    ...
    public static final Property PENNTB_HOME = new Property("PennTreebankHome",
            "/shared/corpora/corporaWeb/treebanks/eng/pennTreebank/treebank-3/parsed/mrg/wsj/");
    public static final Property PROPBANK_PARSER = new Property("PropbankHome",
            "/shared/corpora/corporaWeb/treebanks/eng/propbank_1/data");
    public static final Property NOMBANK_HOME = new Property("NombankHome",
            "/shared/corpora/corporaWeb/treebanks/eng/nombank/");


    /**
     * The directory of the sentence and pre-extracted features database (~5G of space required) Not
     * used during test/working with pre-trained models
     */
     ...
     */
    public static final Property INSTANTIATE_PREPROCESSOR = new Property("instantiatePreprocessor",
            FALSE);

    /**
     * whether to use lazy initialization to defer loading of models etc until actually needed
     */
    public static final Property LAZILY_INITIALIZE = new Property(
            AnnotatorConfigurator.IS_LAZILY_INITIALIZED.key, FALSE);
    ...
    public ResourceManager getDefaultConfig() {
        Property[] properties =
                {USE_CURATOR, DEFAULT_PARSER, NUM_FEX_THREADS, PENNTB_HOME, PROPBANK_PARSER,
                        NOMBANK_HOME, CACHE_DIR, MODEL_DIR, OUTPUT_DIR, ILP_SOLVER,
                        INSTANTIATE_PREPROCESSOR, LAZILY_INITIALIZE, SRL_TYPE};
        return (new LearnerConfigurator().getConfig(new ResourceManager(
