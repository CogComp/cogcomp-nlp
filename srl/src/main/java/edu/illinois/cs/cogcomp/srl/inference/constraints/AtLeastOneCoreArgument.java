...
/**
 * Every verb should have at leaast one argument that is one of A0, A1 or A2. Holds only for verb.
 * 
...
public class AtLeastOneCoreArgument extends SRLILPConstraintGenerator {
    ...
    public List<ILPConstraint> getViolatedILPConstraints(IInstance ins, IStructure s,
        ...
        for (int predicateId = 0; predicateId < x.numPredicates(); predicateId++) {
            ...
            list.addAll(addPredicateConstraints(manager, variables, manager.getCoreArguments(),
                    predicateId, xp, yp));
        }
