...
/**
 * This constraint says that a label can be SUP (or C-SUP) ONLY if it is either a verb or a Nominal
 * predicate. We assume that the predicates are fixed.
 *
 * @author svivek
 */
public class SupportVerbConstraint extends SRLILPConstraintGenerator {
    ...
    public List<ILPConstraint> getILPConstraints(IInstance ins,
        ...
        for (int predicateId = 0; predicateId < x.numPredicates(); predicateId++) {
            predicatePosition.add(x.predicates.get(predicateId).getSenseInstance().getSpan()
                    .getFirst());
        }

        for (int predicateId = 0; predicateId < x.numPredicates(); predicateId++) {
            SRLPredicateInstance xp = x.predicates.get(predicateId);
            TextAnnotation ta = xp.getSenseInstance().getConstituent().getTextAnnotation();

            int predicate = xp.getSenseInstance().getSpan().getFirst();
            for (int candidateId = 0; candidateId < xp.getCandidateInstances().size(); candidateId++) {

                int candidatePosition =
                        xp.getCandidateInstances().get(candidateId).getSpan().getFirst();

                String pos = WordHelpers.getPOS(ta, candidatePosition);
                if (POSUtils.isPOSVerb(pos))
                    continue;

                if (predicatePosition.contains(candidatePosition) && candidatePosition != predicate)
                    continue;

                int supportVar =
                        getArgumentVariable(variables, manager.getPredictedViewName(), predicateId,
                                candidateId, "SUP");
                ...
                if (cSupportVar >= 0) {
                    ...
                    list.add(new ILPConstraint(new int[] {supportVar, cSupportVar}, new double[] {
                            1, 1}, 0, ILPConstraint.EQUAL));
                } else {
                    list.add(new ILPConstraint(new int[] {supportVar}, new double[] {1}, 0,
                            ILPConstraint.EQUAL));
                }
    ...
    @Override
    public List<ILPConstraint> getViolatedILPConstraints(IInstance x, IStructure y,
            InferenceVariableLexManager variables) {
