...
public class FramesManager {
    ...
    private static final Set<String> prepositions = new HashSet<>(Arrays.asList(new String[] {
            "about", "above", "across", "after", "against", "along", "among", "around", "as", "at",
            "before", "behind", "beneath", "beside", "between", "by", "down", "during", "for",
            "from", "in", "inside", "into", "like", "of", "off", "on", "onto", "over", "round",
    ...
    private void addRoleSets(String file, String lemma, FrameData fData, NodeList roleSets) {
        for (int i = 0; i < roleSets.getLength(); i++) {
            ...
            assert sense.startsWith(IOUtils.stripFileExtension(file)) || sense.startsWith(lemma) : lemma
                    + "\t" + sense;
            ...
            for (int j = 0; j < roles.getLength(); j++) {
                ...
                for (int roleId = 0; roleId < elementsByTagName.getLength(); roleId++) {
                    String vntheta =
                            ((Element) (elementsByTagName.item(roleId))).getAttribute("vntheta");
    ...
    /**
     * Use this to create the compact files used during inference. NB: You need to have access to
     * the Propbank and Nombank frame files
     */
