...
public class SRLProperties {
    ...
    /**
     * If SRLProperties has not yet been instantiated, initialize new instance with the non-default
     * parameters specified in the configFile named in the argument, and default parameters
     * otherwise. If a null string is given as the argument, all default parameters are used.
     * 
    ...
    public static SRLProperties getInstance() {
        if (theInstance == null) {
            System.out
                    .println("SRL config not initialized. Instantiating with default parameters.");
            try {
                initialize();
            } catch (Exception e) {
    ...
    String getFeatureCacheFile(SRLType SRLType, Models type, String featureSet, String parser,
            Dataset dataset) {
        return this.config.getString("CacheDirectory") + "/features." + SRLType.name() + "."
                + type.name() + "." + featureSet + "." + dataset + "." + parser + ".cache";
    }

    String getPrunedFeatureCacheFile(SRLType SRLType, Models type, String featureSet, String parser) {
        return this.config.getString("CacheDirectory") + "/features." + SRLType.name() + "."
                + type.name() + "." + featureSet + "." + parser + ".pruned.cache";
    }
    ...
    public int getNumFeatExtThreads() {
        return config.getInt("NumFeatExtThreads");
    }
        ...
        switch (solver) {
            case "Gurobi":
                if (isEvaluating)
                    ...
                    return ILPSolverFactory.SolverType.Gurobi;
            case "OJAlgo":
