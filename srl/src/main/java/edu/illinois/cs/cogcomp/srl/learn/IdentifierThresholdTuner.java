...
public class IdentifierThresholdTuner {
    ...
    public Pair<Double, Double> tuneIdentifierScale(List<Double> A, List<Double> B)
            throws Exception {
        ...
        System.out.println();
        System.out.println("Based on F" + n_F + " measure, recommended (A, B) = " + maxer);

        System.out.println("Top 100 values");

        System.out.println("(A, B)\ttotalGold\ttotalPredicted\tcorrect\tP\tR\tF" + n_F);
    ...
    private Map<Pair<Double, Double>, IntPair> getPerformance(List<Double> A, List<Double> B,
            final List<Pair<Double, Boolean>> scores) throws InterruptedException,
            ExecutionException {
        ...
        for (List<Double> element : Permutations.crossProduct(Arrays.asList(A, B))) {

            final double a = element.get(0);
            final double b = element.get(1);
            FutureTask<Pair<Pair<Double, Double>, IntPair>> task =
                    new FutureTask<>(new Callable<Pair<Pair<Double, Double>, IntPair>>() {

                        @Override
                        public Pair<Pair<Double, Double>, IntPair> call() throws Exception {
    ...
    private Pair<Pair<Double, Double>, IntPair> getPerformance(double A, double B,
            List<Pair<Double, Boolean>> scores) {
