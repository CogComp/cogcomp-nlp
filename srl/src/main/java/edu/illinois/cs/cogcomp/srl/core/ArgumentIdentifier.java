...
/**
 * The argument identifier, that uses the scores from the learned model and scales it for high
 * recall. The scaling function has two parameters (A,B) and the scaled score is A *
 * classifier-score + B. The scaled decision is true if this is positive.
 *
...
public class ArgumentIdentifier extends Predicate<SRLMulticlassInstance> {
    ...
    public double getIdentifierScaledScore(SRLMulticlassInstance x) throws Exception {
    ...
    public double getIdentifierRawScore(SRLMulticlassInstance x) throws Exception {
        ...
        double score1 =
                w.dotProduct(x.getCachedFeatureVector(Models.Identifier),
                        1 * manager.getModelInfo(Models.Identifier).getLexicon().size());
