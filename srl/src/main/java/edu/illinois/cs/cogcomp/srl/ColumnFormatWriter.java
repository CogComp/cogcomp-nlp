...
/**
 * Prints text annotation formatted with one word per line as follows. <blockquote> <code>
...
public class ColumnFormatWriter {
    ...
    public ColumnFormatWriter(String parseViewName, String predicateArgumentViewName) {
    ...
    public void transform(Iterable<TextAnnotation> reader, PrintWriter out) throws Exception {
    ...
    public void printPredicateArgumentView(PredicateArgumentView pav, PrintWriter out) {
    ...
    /**
     * Return a table. Numrows = number of words. Num Cols depends on how many predicate arg
     * relations we have
     */
    ...
    private static String[] getNEData(TextAnnotation ta) {
        ...
        Collections.sort(nerConstituents, TextAnnotationUtilities.constituentStartComparator);
    ...
    private void convertPredicateArgView(TextAnnotation ta, PredicateArgumentView pav,
            List<String[]> columns, boolean addSense) {
    ...
    private void addPredicateInfo(List<String[]> columns, List<Constituent> predicates, int size,
            boolean addSense) {
    ...
    private String[] addPredicateArgInfo(Constituent predicate, List<Relation> args, int size) {
        ...
        for (int sentenceId = 0; sentenceId < ta.getNumberOfSentences(); sentenceId++) {
            ...
            if (treeLines.length != sentence.size()) {
                ...
                throw new IllegalStateException("Expected " + sentence.size()
                        + " tokens, but found " + treeLines.length + " tokens in the tree");
            }
