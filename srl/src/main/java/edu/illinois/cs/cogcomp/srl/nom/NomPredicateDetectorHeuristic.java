...
public class NomPredicateDetectorHeuristic extends AbstractPredicateDetector {
    ...
    static {
        ...
        pluralLemmas.addAll(Arrays.asList("filers", "hundreds", "thousands", "millions",
                "billions", "tens"));
    ...
    public NomPredicateDetectorHeuristic(NomSRLManager manager) throws EdisonException {
        super(manager);
    ...
    private Option<String> testWithDelim(String token, char delim) {
        ...
        if (debug) {
            System.out.println("Testing with delimiter: " + delim + ". Token=" + token);
        }
