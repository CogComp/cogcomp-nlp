...
public abstract class SRLManager {
    ...
    /**
     * This class works with a parse trees from this parser, which controls both candidate
     * extraction and feature generation.
     */
    ...
    protected SRLManager(boolean trainingMode, String defaultParser) throws Exception {
        ...
        allArgumentsSet =
                Collections.unmodifiableSet(new TreeSet<>(Arrays.asList(getArgumentLabels())));
    ...
    private void initializeFeatureManifest(String defaultParser) {
        ...
        FeatureManifest.setTransformer("parse-right-sibling",
                FeatureGenerators.getParseRightSibling(defaultParser));
    ...
    public int getArgumentId(String label) {
        ...
        if (argToId.containsKey(label))
            ...
            log.debug(label + " is not a valid argument. Expecting one of " + this.argToId.keySet()
                    + ", replacing with " + NULL_LABEL);
                ...
                if (this.identifier == null) {
                    ...
                    try {
                        ...
                        if (!IOUtils.exists(getIdentifierScaleFile())) {
                            ...
                            List<URL> urls =
                                    IOUtils.lsResources(SRLManager.class, getIdentifierScaleFile());
                            if (urls.size() == 0)
                        ...
                        this.identifier =
                                new ArgumentIdentifier(pair.getFirst(), pair.getSecond(), this);
    ...
    private String getFeatureIdentifier(Models type) {
        return getModelInfo(type).featureManifest.getIncludedFeatures().replaceAll("\\s+", "")
                .hashCode()
                + "";
    }
    ...
    public String getLexiconFileName(Models m) {
        return properties.getModelsDir() + "/" + getSRLType() + "." + m + "." + defaultParser
                + ".lex";
    }
    ...
    /**
     * Returns the set of legal arguments for the given lemma. This function uses the frame files to
     * get the list of valid core arguments. All the modifiers are treated as legal arguments. In
     * addition, all C-args and R-args of legal core/modifier arguments are also considered legal.
     * For unknown predicates, all arguments are legal.
     */
    public Set<String> getLegalArguments(String lemma) {
        if (knownLegalArguments.hasLegalArguments(lemma)) {
            Set<String> set = new HashSet<>();

            set.addAll(Arrays.asList("AM-ADV", "AM-DIS", "AM-LOC", "AM-MNR", "AM-MOD", "AM-NEG",
                    "AM-TMP"));
    ...
    /**
     * Get valid arguments for each sense of a given lemma from the frame files. For unknown
     * predicates, only the sense 01 is allowed with all arguments
     */
    public Map<String, Set<String>> getLegalLabelsForSense(String lemma) {
        Map<String, Set<String>> map = new HashMap<>();

        if (knownLegalArguments.hasLegalSenses(lemma)
                && knownLegalArguments.hasLegalArguments(lemma)) {
            for (String sense : knownLegalArguments.getLegalSenses(lemma)) {
                Set<String> argsForSense =
                        new HashSet<>(knownLegalArguments.getLegalArgsForSense(lemma, sense));
    ...
    public double[] getScores(SRLMulticlassInstance x, Models type, boolean rescoreInvalidLabels) {
        ...
        for (int label = 0; label < numLabels; label++) {
            ...
            if (!this.isValidLabel(x, type, label) && rescoreInvalidLabels) {
                ...
                scores[label] =
                        w.dotProduct(x.getCachedFeatureVector(type), label
                                * this.getModelInfo(type).getLexicon().size());
            }
        }
    ...
    public SRLILPInference getInference(ILPSolverFactory solver, List<Constituent> predicates)
            throws Exception {
