...
/**
 * This comes from section 7.1 of "Combination Strategies for Semantic Role Labeling": Arguments
 * fror two different verbs can not overlap, but they can embed.
 * <p>
...
public class CrossArgumentExclusiveOverlap extends SRLILPConstraintGenerator {
    ...
    public CrossArgumentExclusiveOverlap(SRLManager manager) {
        super(manager, SRLConstraints.noCrossArgumentExclusiveOverlap.name(), false);
        ...
        for (int predicateId = 0; predicateId < numPredicates; predicateId++) {
            ...
            for (int candidateId = 0; candidateId < numCandidates; candidateId++) {
                ...
                addCandidateConstraints(predicateId, candidateId,
                        candidateInstances.get(candidateId).getConstituent(), x, y, constraints,
                        variables);
    ...
    private void addCandidateConstraints(int predicateId, int candidateId, Constituent candidate,
            SRLSentenceInstance x, SRLSentenceStructure y, List<ILPConstraint> constraints,
            InferenceVariableLexManager variables) {

        int nullVar =
                this.getArgumentVariable(variables, manager.getPredictedViewName(), predicateId,
                        candidateId, SRLManager.NULL_LABEL);

        for (int otherPredicateId = predicateId + 1; otherPredicateId < x.numPredicates(); otherPredicateId++) {

            List<SRLMulticlassInstance> candidateInstances =
                    x.predicates.get(otherPredicateId).getCandidateInstances();
            int numCandidates = candidateInstances.size();
            ...
            for (int otherCandidateId = 0; otherCandidateId < numCandidates; otherCandidateId++) {
                ...
                int otherLabel = -1;
                if (y != null) {
                    otherLabel = otherPredicateStructure.getArgLabel(candidateId);

                    if (otherLabel == nullId)
                        continue;
                }

                if (!Queries.exclusivelyOverlaps(candidate).transform(otherConstituent))
                    continue;

                int otherNullVar =
                        this.getArgumentVariable(variables, manager.getPredictedViewName(),
                                otherPredicateId, otherCandidateId, SRLManager.NULL_LABEL);

                // at least one of these two variables should be null.

                constraints.add(new ILPConstraint(new int[] {nullVar, otherNullVar}, new double[] {
                        1, 1}, 1, ILPConstraint.GREATER_THAN));
            }
