...
/**
 * Collection of regex es which are not currently used
 * The functions are not expected to work now, kept only as an archive
 * @author subhroroy
 *
 */
...
public class Archive {
	...
	ordinalDanglersPat, danglersPat, danglingAddPat, danglingMultPat, usdPat, feetPat, inchPat,
	separateSuffixPat, allPat, mForMillionPat, prefixUsdPat, prefixGbpPat, prefixEuroPat,
	aAnPat, digitPat, notSurePat, anAPerPat, justAnAPerPat, frac1Pat;	
	...
	public static void initialize() {
		...
		dollarRewritePat = Pattern.compile(
				"((?:U.?S.?s*(?:dollars?|dlrs?|US\\$)|dlrs?|dollars?|US\\s*\\$USD|USD))", 
				Pattern.CASE_INSENSITIVE);
		
		removesPat = Pattern.compile(
				"^\\s*(?:a|1)\\s+|metric|[\"\',{}]|(?:\\W|^)th(?:\\W|$)", 
				Pattern.CASE_INSENSITIVE);
		
		inAPat = Pattern.compile(
				"\\s+(in\\s+/)\\s+", 
				Pattern.CASE_INSENSITIVE);
		
		ofPat = Pattern.compile("^of\\s", Pattern.CASE_INSENSITIVE);
		
		dateCheckPat = Pattern.compile("((\\d{1,4}|"+Date.MONTH+
				")\\s*[/\\-\\.]\\s*(\\d\\d?|"+Date.MONTH+
				")\\s*[/\\-\\.]\\s*(\\d{1,4}|"+Date.MONTH+"))", 
				Pattern.CASE_INSENSITIVE);
		
		yuckyHyphenPat = Pattern.compile("(.*?)(\\d+.*?)(?:%u2013|-)(.*?\\d+.*?)", 
				Pattern.CASE_INSENSITIVE);
		...
		monthOfPat = Pattern.compile( "(" + Date.MONTH + ")\\s*of\\s*(\\d{4}|\\d{2})", 
				Pattern.CASE_INSENSITIVE);
		fracToDoublePat = Pattern.compile("(?:("+Numbers.NUMBER+")\\s*\\-?\\s*("+
				Numbers.FRACTION+")(s?)|(?:\\W|^)(?:an|a|one)\\s*-?\\s*("+Numbers.FRACTION+"))",
					Pattern.CASE_INSENSITIVE);
		    
	    aOrAnThenNumberPat = Pattern.compile("(?:\\W|^)(an|a)\\s*("+
	    		Numbers.ANYNUMBERLIKETHING+")(?:\\W|$)", 
				Pattern.CASE_INSENSITIVE);
	    
	    numberWordHyphenationPat = Pattern.compile("(\\s*("+Numbers.NUMBER+"|"+Numbers.FRACTION+
				")\\s*-\\s*("+Numbers.NUMBER+"|"+Numbers.FRACTION+"|"+Numbers.ORDINAL+
				")\\s*)", Pattern.CASE_INSENSITIVE);
	    
	    ordinalDanglersPat = Pattern.compile("(([^\\s]|^)\\s+("+Numbers.ORDINAL+")[^s](.*))", 
				Pattern.CASE_INSENSITIVE );
	    
	    danglersPat = Pattern.compile("(([^a-z]|^)("+Numbers.NUMBER+"|"+Numbers.FRACTION+
				")(\\W|$))", Pattern.CASE_INSENSITIVE );
	    
	    danglingAddPat = Pattern.compile(
				"("+Numbers.DECIMAL+")\\s+("+Numbers.DECIMAL+")", 
				Pattern.CASE_INSENSITIVE );
	    
	    danglingMultPat = Pattern.compile(
				"("+Numbers.DECIMAL+")\\s+("+Numbers.DECIMAL+")", 
				Pattern.CASE_INSENSITIVE);
	    
	    usdPat = Pattern.compile("(USD|US\\$|\\$US|US dlrs\\.?|dollars?|dlrs?\\.?|\\$)", 
				Pattern.CASE_INSENSITIVE );
		...
		separateSuffixPat = Pattern.compile("("+Numbers.NUMBER+"|"+Numbers.FRACTION+")s", 
				Pattern.CASE_INSENSITIVE );
		...
		prefixUsdPat = Pattern.compile("(US\\$|\\$)\\s*([\\d\\.]+)", 
				Pattern.CASE_INSENSITIVE );
		prefixEuroPat = Pattern.compile("Euros\\s*([\\d\\.]+)", 
				Pattern.CASE_INSENSITIVE );
		...
		frac1Pat = Pattern.compile(
				"(?:\\s*(\\d+/\\d+/\\d+)\\s*|(?=[^/\\.\\-0-9])\\s+(\\d+\\.\\d+|\\d+)"
				+ "\\s*/\\s*(\\d+)(?:\\W+|$))",
				Pattern.CASE_INSENSITIVE);	

	    intWillBeIntPat = Pattern.compile("\\d+\\.\\d+", Pattern.CASE_INSENSITIVE );
	    removeScientificPat = Pattern.compile("([\\-0-9]+)(?:\\.([0-9]*))?e(\\-|\\+|)(\\d+)", 
	    		Pattern.CASE_INSENSITIVE );
	    standardizeDecimalPat = Pattern.compile("(\\d),(\\d\\d)([^\\d]|$)", 
	    		Pattern.CASE_INSENSITIVE );
		
	    pryNumbersFromLettersPat = Pattern.compile("(\\d+)([a-zA-Z]+)", Pattern.CASE_INSENSITIVE );
		pryNumbersFromSymbolsPat = Pattern.compile("([^\\d,\\./]+)(\\d+)", 
				Pattern.CASE_INSENSITIVE );
		...
		if( matcher.find() ){
			if( matcher.group(1) != null )
				...
				return str.replace(matcher.group(), " "+
						(Float.parseFloat(matcher.group(2).trim())*
						1.0/Float.parseFloat(matcher.group(3).trim()))+" ");
		}
		return str;
	}
	...
	public static String Frac2double( String str ) {
		...
		if(matcher.find()){
			...
			if( matcher.group(3).equals("")==false){
				...
				return str.replace(matcher.group(), ""+
				(Numbers.str2num.get(matcher.group(1).toLowerCase())*
						1.0/Numbers.fractions.get(matcher.group(2).toLowerCase()))); 
			}
			else{
	...
	public static String OrdinalDanglers(String str) {
		...
		if(matcher.find()){
			...
			if(x.toLowerCase().equals("second") == false)
				return str.replace(matcher.group(),before+" "+
						Numbers.ordinals.get(x.toLowerCase())+" "+after+" ");
	...
	public static String Danglers( String str ){
		...
		if( matcher.find()){
			...
			if( Numbers.str2num.containsKey(x.toLowerCase()))
				return str.replace(matcher.group(),junkA+Numbers.str2num.get(
						x.toLowerCase())+junkB);
			else if( Numbers.fractions.containsKey(x.toLowerCase()))
				return str.replace(matcher.group(),junkA+(1/Numbers.fractions.get(
						x.toLowerCase()))+junkB);
			else
				return str.replace(matcher.group(),chunk);
		}
...
//				+ "Order:"+order);
	...
	public static String collapseConsecutiveNumbers( String phrase ){
		...
		List<String> splitup = new ArrayList<String>( 
...
//			phrase = phrase.replace( matcher.group(), matcher.group(1)+" GBP "+matcher.group(3) );
	...
	public static String postprocess( String phrase ) {
		...
		if(matcher.find())
			phrase = phrase.replace( matcher.group(), 
					matcher.group(1)+"s "+matcher.group(2));
		// tighten up spaces
	...
	public static String RemoveScientificNotation( String str ) {
		...
		if(matcher.find()) {
			...
			if(sign.equals("-"))
				...
				trail_digits+=zeros;
				return str.replace( matcher.group(), 
						lead_digit+trail_digits.substring(0, exp)
	...
	//	System.err.println("Before little m: "+phrase);
		...
		if(matcher.find())
			phrase = phrase.replace( matcher.group(), "million "+matcher.group(1)+" "+
		matcher.group(2));
...
//	    test("an eighth", "1/8");
//	    test("AN eiGHth", "1/8");
//	    test("she only meets half of the requirements.", "she only meets 0.5 of the requirements.");
