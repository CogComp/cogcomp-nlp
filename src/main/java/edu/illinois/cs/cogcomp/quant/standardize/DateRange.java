...
public class DateRange implements Serializable{
	...
	public static void initialize() {
	    thisNextDatePat = Pattern.compile(
				"(this|next)\\s*"
				+ "(week|fort\\W*nights?|centur(?:ys?|ies)|decades?|months?"
				+ "|weeks?|days?|years?)",
				Pattern.CASE_INSENSITIVE);
	    periodPat = Pattern.compile(
				"(\\d{4}|\\d{2})\\s*(?:st|nd|rd|th|)\\W*?(century|cent|s)", 
				Pattern.CASE_INSENSITIVE);
	    pastLastDatePat = Pattern.compile(
				"(past|last|since)\\s*(\\d+|a)?\\s*(fort\\W*nights?|centur"
				+ "(?:ys?|ies)|decades?|months?|weeks?|days?|years?)", 
				Pattern.CASE_INSENSITIVE);
	    fromToDatePat = Pattern.compile(
				"(?:from|between|start(?:ing|s)?|begin(?:ing|s)?)"
				+ "(.+?)\\s"
				+ "(?:-|and|to|thru|through|until|and\\s*ending|finish[a-z]*|end[a-z]*)\\s"
				+ "(.+)",
				Pattern.CASE_INSENSITIVE);
	    fromDatePat = Pattern.compile(
				"(?:since|from|start(?:ing|s)?|begin(?:ing|s)?)"
				+ "(.+?)",
				Pattern.CASE_INSENSITIVE);
	    justYearPat1 = Pattern.compile(
				"([\\dX]{4})-([\\dX]{4})",
				Pattern.CASE_INSENSITIVE);
	    justYearPat2 = Pattern.compile(
				"([\\dX]{4})-([\\dX]{2})",
				Pattern.CASE_INSENSITIVE);
	}
	
	public static DateRange thisNextDate( String str ){
		/*Modified dates examples:
        this weekend, this monday
        Does not support double modifiers: weekend after next...   months:  this March
        */
	...
	public static DateRange pastLastDate( String str ) {
		...
		if(matcher.find()){
			...
			return new DateRange(
					Date.getRelativeDate(unit,-nm,new Date(
							Date.presentDate.get(Calendar.YEAR),
							Date.presentDate.get(Calendar.MONTH),
							Date.presentDate.get(Calendar.DAY_OF_MONTH))),
	...
	public static DateRange fromDate( String str ) {
		...
		if(matcher.find()){
			...
			return new DateRange(date, new Date(
					Date.presentDate.get(Calendar.YEAR),
					Date.presentDate.get(Calendar.MONTH),
					Date.presentDate.get(Calendar.DAY_OF_MONTH)));
		}
