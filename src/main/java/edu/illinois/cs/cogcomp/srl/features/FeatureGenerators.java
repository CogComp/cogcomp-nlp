...
public class FeatureGenerators {
	...
	public static final FeatureInputTransformer candidateBoundaryTransformer = new FeatureInputTransformer() {
	...
	public static FeatureExtractor ppFeatures(final String parseViewName) {
		return new FeatureExtractor() {
			...
			@Override
			public Set<Feature> getFeatures(Constituent c)
		...
		return new FeatureInputTransformer() {
			...
			public List<Constituent> transform(Constituent input) {
				...
				try {
					...
					if (in.size() > 0) {
						List<Relation> outgoingRelations = in.get(0)
								.getSource().getOutgoingRelations();
						int id = -1;
						...
						if (id >= 0 && id + 1 < outgoingRelations.size())
							siblings.add(outgoingRelations.get(id + 1)
									.getTarget());
					}
