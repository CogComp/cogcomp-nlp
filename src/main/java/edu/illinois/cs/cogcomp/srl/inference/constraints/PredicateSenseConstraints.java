...
public class PredicateSenseConstraints extends SRLILPConstraintGenerator {
	...
	public List<ILPConstraint> getViolatedILPConstraints(IInstance ins,
		...
		for (int predicateId = 0; predicateId < x.numPredicates(); predicateId++) {
			...
			list.addAll(getPredicateSenseConstraints(manager, predicateId, xp,
					yp, variables));
		}
	...
	private List<ILPConstraint> getPredicateSenseConstraints(
		...
		Map<String, Set<String>> validLabelsForSense = manager
				.getLegalLabelsForSense(lemma);
		...
		for (int candidateId = 0; candidateId < numCandidates; candidateId++) {
			for (String sense : validSenses) {
				...
				String senseVarName = SRLILPInference
						.getSenseVariableIdentifier(type, predicateId, sense);
				...
				vars[1] = getArgumentVariable(variables, type, predicateId,
						candidateId, SRLManager.NULL_LABEL);
				coefs[1] = 1.0;

				int i = 2;
				for (String validLabel : labels) {
					int var = getArgumentVariable(variables, type, predicateId,
							candidateId, validLabel);
					vars[i] = var;
					coefs[i] = 1.0;
					i++;
				}
				...
				if (vars.length > 0) {
					...
					list.add(new ILPConstraint(vars, coefs, 0,
