...
public class Main {
	...
	private static void pruneFeatures(int numConsumers, SenseManager manager, FeatureVectorCacheFile featureCache,
	...
	private static FeatureVectorCacheFile preExtract(int numConsumers, SenseManager manager, Dataset dataset,
													 String cacheFile) throws Exception {
	...
	public static void evaluate() throws Exception {
		...
		while (dataset.hasNext()) {
			TextAnnotation ta = dataset.next();
			if (!ta.hasView(SenseManager.getGoldViewName())) continue;
	...
	private static void evaluateSense(TokenLabelView gold, TokenLabelView prediction, ClassificationTester tester) {
