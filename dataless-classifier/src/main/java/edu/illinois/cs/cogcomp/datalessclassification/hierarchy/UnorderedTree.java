 ...
 *   -- allows some data to be associated with Edges, 
 *   		which was not immediately required but is kind of a forward planning for the project
 *   
 * @author shashank
 */

public class UnorderedTree<V extends Serializable, E extends Serializable> extends AbstractTypedGraph<V, E> implements Tree<V, E> {
    ...
    /**
     * Returns a set of vertex's child vertices.
     * If the vertex has no children then an empty set will be returned.
     */
    @Override
     ...
     * with the specified parent vertex {@code parent}.
     * 
    ...
    public boolean addEdge (E e, V parent, V child) {
    	...
    	if (containsEdge(e) == true)
    		if (endpoints.equals(edge_vpairs.get(e)) == false)
    	...
    	if (outedges == null) {
    		parent_data.child_edges = new HashSet<E>();    		
    ...
    public V getDest (E directed_edge) {
        if (containsEdge(directed_edge) == false)
            return null;
    ...
    /**
     * @return Empty Set if the <code>vertex</code> is the root, null if the vertex is 
     * not an element of this tree, and the Parent wrapper in a set otherwise
     */
    @Override
    ...
    /**
     * Returns true iff <code>v1</code> is the child of <code>v2</code>.
     * Note that if <code>v2</code> is a leaf node and <code>v1</code> is <code>null</code>,
     * this method returns <code>true</code>.
     */
    @Override
	...
	public boolean equals (Object o) {
    	if ((o instanceof UnorderedTree<?, ?>) == false)
    		return false;
		...
		for (E edge : this.edge_vpairs.keySet()) {
			...
			if (that.edge_vpairs.containsKey(edge) == false)
				return false;
			
			Pair<V> thatPair = that.edge_vpairs.get(edge);
			
			if (thisPair.equals(thatPair) == false)
				return false;
		}
		...
		for (V vertex : this.vertex_data.keySet()) {
			...
			if (that.vertex_data.containsKey(vertex) == false)
				return false;
			
			VertexData thatData = that.vertex_data.get(vertex);
			
			if (thisData.equals(thatData) == false)
				return false;
		}
		...
		if (this.root.equals(that.root) == false)
			return false;
...
//		out.writeInt(edge_vpairs.size());
//		
//		for (E edge : edge_vpairs.keySet()) {
//			out.writeObject(edge);
//			out.writeObject(edge_vpairs.get(edge));
//		}
//		
//		out.writeInt(vertex_data.size());
//		
//		for (V vertex : vertex_data.keySet()) {
//			out.writeObject(vertex);
//			out.writeObject(vertex_data.get(vertex));
//		}
//		
//		out.writeInt(height);
...
//		int size = in.readInt();
//		
//		edge_vpairs = new HashMap<E, Pair<V>>(size);
//		
//		for (int i = 0; i < size; i++) {
//			E edge = (E) in.readObject();
//			Pair<V> pair = (Pair<V>) in.readObject();
//			edge_vpairs.put(edge, pair);
//		}
//		
//		size = in.readInt();
//		
//		vertex_data = new HashMap<V, VertexData>(size);
//		
//		for (int i = 0; i < size; i++) {
//			V vertex = (V) in.readObject();
//			VertexData data = (VertexData) in.readObject();
//			vertex_data.put(vertex, data);
//		}
//		
//		height = in.readInt();
//		root = (V) in.readObject();
//	}
	...
	protected class VertexData implements Serializable {
		...
		public boolean equals (VertexData that) {
			...
			for (E edge : this.child_edges) {
				if (that.child_edges.contains(edge) == false)
					return false;
			}
			
			if (this.parent_edge.equals(that.parent_edge) == false)
				return false;
