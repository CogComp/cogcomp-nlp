...
/**
 * The {@link edu.illinois.cs.cogcomp.annotation.AnnotatorService} object with <i>most</i> of the annotators available in CCG Curator.
 * <br/>
 * Do not call this class directly; instead, use {@link edu.illinois.cs.cogcomp.curator.CuratorFactory}
 * <br/>
...
public class CuratorAnnotatorService implements AnnotatorService {
     ...
     * @see edu.illinois.cs.cogcomp.curator.CuratorAnnotatorService#CuratorAnnotatorService(edu.illinois.cs.cogcomp.core.utilities.ResourceManager)
     */
    ...
    /**
     * Builds an {@link edu.illinois.cs.cogcomp.annotation.AnnotatorService} object that contains all {@code Curator} components under {@link edu.illinois.cs.cogcomp.core.datastructures.ViewNames}.
     * (to access the rest of the {@code Curator} annotators, you will need to requrest it manually, by altering
     * {@link edu.illinois.cs.cogcomp.curator.CuratorClient#addRecordViewFromCurator}).
     * <p/>
     * Each {@code Curator} internal component is wrapped in a {@link edu.illinois.cs.cogcomp.curator.CuratorAnnotator} that automates the
     * retrieval of {@code requiredViews} and {@code viewName}. The current set of components is built using
     * the dependencies and view names found in
     * {@code /shared/trollope/curator/dist/configs/annotators-trollope.xml, Mar 12 09:12}.
     *
     * @param rm ResourceManager with properties for the {@link edu.illinois.cs.cogcomp.curator.CuratorClient} and caching behavior
     */
    protected CuratorAnnotatorService(ResourceManager rm) {
        ...
        annotators.add(new CuratorAnnotator(curatorClient, ViewNames.PARSE_STANFORD, requiredViews));
        //TODO Curator doesn't return this at the moment
        //annotators.add(new CuratorAnnotator(curatorClient, ViewNames.PARSE_STANFORD_KBEST, requiredViews));
        annotators.add(new CuratorAnnotator(curatorClient, ViewNames.DEPENDENCY_STANFORD, requiredViews));
        annotators.add(new CuratorAnnotator(curatorClient, ViewNames.PARSE_CHARNIAK, requiredViews));
        //TODO Curator doesn't return this at the moment
        //annotators.add(new CuratorAnnotator(curatorClient, ViewNames.PARSE_CHARNIAK_KBEST, requiredViews));
        ...
        annotators.add(new CuratorAnnotator(curatorClient, ViewNames.PARSE_BERKELEY, requiredViews));

        requiredViews = new String[]{ViewNames.SENTENCE, ViewNames.TOKENS, ViewNames.POS, ViewNames.NER_CONLL};
