 ...
 *    refer to these expanded forms.
 *
 * Structure consists of one or more "post" elements, each possibly containing one or more
 *    "quote" elements (which may be nested) and which may have other tags (image files and
 *    other url-like stuff, possibly html formatting).
 *
 * This reader (initial implementation) tries to clean up text as much as possible while
 *   preserving character offsets of the original text. This is achieved by whitespacing
 *   the xml/other tags; the Illinois Tokenizer should be able to handle this in an
 *   offset-preserving way.
 *
 ...
 *
 * When trying to align annotations to the original file, beware the following annotation
 *   property (explained in the README from the corpus:
 *
 <quote>Because each CMP document is extracted verbatim from source XML files,
