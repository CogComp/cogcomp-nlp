...
public class AceFileProcessor {
    ...
    public ACEDocument processAceEntry(File subFolderEntry, ACEDocumentAnnotation annotationACE,
        ...
        if (null != paraList) {
            for (int j = 0; j < paraList.size(); ++j) {
                if (paraList.get(j).getFirst().equals("text")) {
                    String text =
                            StringCleanup.normalizeToAscii(paraList.get(j).getSecond().content);
                    ...
                    TextAnnotation ta =
                            taBuilder.createTextAnnotation(corpusID, annotationACE.id + "_" + j,
                                    text);
                    alignTokenToCharOffset(contentRemovingTags, text,
                            paraList.get(j).getSecond().offsetFilterTags, ta);
    ...
    /**
     * generates an additional view of Tokens with character offsets as string attributes, where
     * those character offsets correspond to offsets in the original source document
     *
    ...
    public static void alignTokenToCharOffset(String origDocWithoutTags, String paraText,
        ...
        SpanLabelView offsetView =
                new SpanLabelView(TOKEN_WITH_CHAR_OFFSET, "Default", ta, 1.0, false);
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            String testText =
                    origDocWithoutTags.substring(textOffset + start, textOffset + end + 1);
     ...
     * WARNING: currently, only populates Entity view (name: EventConstants.NER_ACE_COARSE) quantity
     * view ( name: EventConstants.NER_ACE_QUANTITY) and timex view (name:
     * EventConstants.NER_ACE_TIME)
     *
     * @param aceDocument
     * @return
     */
        ...
        for (ACETimeEx e : times) {
            ...
            for (ACETimeExMention m : e.timeExMentionList) {
                ...
                if (!useFilter || !isFiltered(m.extent)) {
                    ...
                    if (null == ta) // can happen: the text we read doesn't contain all mentions,
                                    // apparently.
                        continue;
                    ...
                    {
                        Constituent c =
                                new Constituent(type, 1.0, EventConstants.NER_ACE_TIME, ta,
                                        taTokenOffsets.getFirst(), taTokenOffsets.getSecond());
        ...
        for (ACEValue e : quantities) {
            ...
            for (ACEValueMention m : e.valueMentionList) {
                ...
                if (!useFilter || !isFiltered(m.extent)) {
                    ...
                    {
                        Constituent c =
                                new Constituent(type, 1.0, EventConstants.NER_ACE_QUANTITY, ta,
                                        taTokenOffsets.getFirst(), taTokenOffsets.getSecond());
    ...
    /**
     * Adds two NE views: a coarse NER view and a fine NER view. The first uses only coarse-grained
     * types. The second uses fine-grained types where available, or the coarse-grained types when
     * fine-grained is not specified.
     *
     * @param aceDocument
     * @param taList
     */
        ...
        for (ACEEntity e : entities) {
            ...
            for (ACEEntityMention m : e.entityMentionList) {
                ...
                if (!useFilter || !isFiltered(m.extent)) {
                    ...
                    {
                        Constituent c =
                                new Constituent(type, 1.0, EventConstants.NER_ACE_COARSE, ta,
                                        taTokenOffsets.getFirst(), taTokenOffsets.getSecond());
                        addConstituentToTaNeMap(taToCoarseNeEntities, ta, c);

                        Constituent f =
                                new Constituent(type, 1.0, EventConstants.NER_ACE_FINE, ta,
                                        taTokenOffsets.getFirst(), taTokenOffsets.getSecond());

                        if (null != fineType)
                            f =
                                    new Constituent(fineType, 1.0, EventConstants.NER_ACE_FINE, ta,
                                            taTokenOffsets.getFirst(), taTokenOffsets.getSecond());
    ...
    private static void processEntities(TextAnnotation ta, List<Constituent> neConstituents,
            String viewName) {
    ...
    private static void addConstituentToTaNeMap(
            Map<TextAnnotation, List<Constituent>> taToNeEntities, TextAnnotation ta, Constituent c) {
     ...
     * @return token offsets of corresponding Constituent from the TextAnnotation, or null if not
     *         found
     */
     ...
     */
    private static TextAnnotation findTextAnnotation(int mentionStart, int mentionEnd,
