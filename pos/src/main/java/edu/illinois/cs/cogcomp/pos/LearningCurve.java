...
/**
 * This program does a brute force search over 3 parameters of the LBJ POS tagger's training
 * regimen, testing each combination of parameter values on the test data found in
 * {@link POSConfigurator#DEV_DATA} after training the learning classifiers from scratch each time.
 * All of the performance numbers computed by this program are stored in files in the
 * <code>curves/</code> subdirectory of the working directory, which must exist prior to running
 ...
 * the search space. The <code>&lt;rounds&gt;</code> parameter is the maximum number of training
 ...
 * this file contains a round number and the performance of the complete tagger after completing
 * that training round.
 *
 * <p>
 * During the search, progress updates are sent to <code>STDOUT</code>. Whenever a better performing
 * classifier is found, the two learning classifiers are saved. Thus, when this program terminates,
 * {@link POSTagger} will implement the best performing classifier arrived at during the search.
 * Finally, the parameter settings that produced the best results are sent to <code>STDOUT</code>
...
public class LearningCurve {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (knownBounds[0] < 0 || knownBounds[1] < knownBounds[0] || unknownBounds[0] < 0
                    || unknownBounds[1] < unknownBounds[0] || rounds < 1 || args.length > 5)
                throw new Exception();
        } catch (Exception e) {
            System.err
                    .println("usage: java edu.illinois.cs.cogcomp.lbj.pos.LearningCurve <knownThicknessLB> \\\n"
        ...
        Parser knownTrainParser =
                new FeatureVectorParser(
                        "src/main/java/edu/illinois/cs/cogcomp/lbj/pos/POSTaggerKnown.ex",
                        "src/main/java/edu/illinois/cs/cogcomp/lbj/pos/POSTaggerKnown.lex");
        Parser unknownTrainParser =
                new FeatureVectorParser(
                        "src/main/java/edu/illinois/cs/cogcomp/lbj/pos/POSTaggerUnknown.ex",
                        "src/main/java/edu/illinois/cs/cogcomp/lbj/pos/POSTaggerUnknown.lex");
        Parser testParser =
                new POSBracketToToken(new POSConfigurator().getDefaultConfig().getString("devData"));
        double bestF1 = 0;
        int bestRounds = 0;
        ...
        for (double k = knownBounds[0]; k <= knownBounds[1]; k += .5)
            for (double u = unknownBounds[0]; u <= unknownBounds[1]; u += .5) {
                ...
                try {
                    out = new PrintStream(new FileOutputStream("curves/curve_" + k + "_" + u));
                } catch (Exception e) {
                    System.err.println("Can't open curves/curve_" + k + "_" + u + " for output: "
                            + e);
                    System.exit(1);
                }

                System.out
                        .println("Training k = " + k + ", u = " + u + " started at " + new Date());
                ...
                for (int i = 0; i < rounds; ++i) {
                    ...
                    for (Object ex = knownTrainParser.next(); ex != null; ex =
                            knownTrainParser.next())
                        knownTagger.learn(ex);
                    POSTaggerKnown.isTraining = false;

                    POSTaggerUnknown.isTraining = true;
                    for (Object ex = unknownTrainParser.next(); ex != null; ex =
                            unknownTrainParser.next())
                        unknownTagger.learn(ex);
                    POSTaggerUnknown.isTraining = false;

                    TestDiscrete results = new TestDiscrete();
                    for (Object ex = testParser.next(); ex != null; ex = testParser.next())
                    ...
                    if ((i + 1) % 5 == 0)
                        System.out.println((i + 1) + " rounds.  Best so far: (" + bestCurrentRound
        ...
        System.out.println("\nBest overall:");
        System.out.println("  Thickness for POSTaggerKnown: " + bestKnownThickness);
        System.out.println("  Thickness for POSTaggerUnknown: " + bestUnknownThickness);
