...
/**
  * This program uses {@link POSTagger} to tag a test set with its
  * predictions, indicating which predictions were right and wrong.  The
  * accuracy of the learned classifier on the test set and the time taken to
  * perform various operations are also reported.  All output is sent to
  * <code>STDOUT</code>.
  *
  * <h4>Usage</h4>
  * <blockquote><code>
  ...
  *
  * <h4>Input</h4>
  * <code>-q</code> mutes the tagged output, leaving only the accuracy report.
  *
  * <h4>Output</h4>
  * One sentence per line, words surrounded by parentheses accompanied by the
  * predicted POS tag, and, immediately preceding each word, either a dash
  * followed by the true label surrounded in square brackets or a plus sign if
  * the prediction was incorrect or correct respectively.
  *
  * @author Nick Rizzolo
 **/
public class POSTag
{
  ...
  public static void main(String[] args)
  {
    ...
    {
      System.err.println(
         "usage: java edu.illinois.cs.cogcomp.lbj.pos.POSTag [-q] <testing set>\n"
    ...
    for (String line = readLine(in); line != null; line = readLine(in))
    {
      ...
      for (Word word = (Word) sentence.get(0); word != null;
           word = (Word) word.next)
      {
        ...
        {
          ++incorrect;
          if (!quiet) System.out.print("-[" + label + "]");
        }

        if (!quiet) System.out.print(word + " ");
      }
  ...
  private static BufferedReader open()
  {
    ...
    try { result = new BufferedReader(new FileReader(testingFile)); }
    catch (Exception e)
    {
