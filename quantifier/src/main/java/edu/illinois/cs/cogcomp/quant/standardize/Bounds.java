...
/**
 * @author Subhro Roy
 * Separate bound from rest of phrase
 */
...
class Bounds{
	...
	public static void initialize() {
		...
		//      BOUNDS
		//***************************************************************************************
		String[] eqList={"exact(?:ly)?","equals?(?:ing)?(?:\\s*to)?",
			"(?:or|and)\\s*(?:more|less|under|below|fewer|higher|older|up|over|above|"
			+ "younger|before|after|prior|under|beneath)",
			"as.*?as"};
		
		String[] incrList = {"(?:increased?|risen|grew|rose)(?:\\s*by)?",
			"gaine?d?","added","(?:in\\s*)?additiona?l?","(?:later|after)\\s*$"};
		
		String[] decrList = {"(?:decreased?|dropped|drop)(?:\\s*by)?","loss",
			        "(?:before)\\s*$", "(?:fell|shr[ia]nk)\\sby" };

		String[] apList = {"(?:just\\s*)?about","around","approx(?:imate(?:ly)?d?)?\\.?",
			"rough(?:ly)?","estimated?","some","close(?:\\s*to|ly)?","near(?:ly)?","almost",
			"or\\s*so",
			"semi","like","quasi(?:\\s*-\\s*)?"};

		String[] ltList = {
			"(?:at\\s*)?(?:most|(?:the)?\\s*highest|(?:a\\s*)?maximum(?:\\s*of)?)",
			"below","(?:or|and)?\\s*(?:less|fewer|lower)(?:\\s*than)?",
			"fewer","before","prior(?:\\s*to)?",
			"under","beneath","as\\s*(?:many|much|long|late|far|heavy|dense|high|big|huge|"
			+ "large|old)\\s*as",
			"(?:or\\s*)?younger(?:\\s*than)?","up\\s*to"};
		...
		String[] gtList = {
			...
			"following","upwards(?:\\s*of)?","(?:a\\s*)?minimum(?:\\s*of)?",
		...
		ltNegatedPat = Pattern.compile(keyword_expr(ltNegatedList), 
				Pattern.CASE_INSENSITIVE);
		gtPat = Pattern.compile(keyword_expr(gtList), 
				Pattern.CASE_INSENSITIVE);
		gtNegatedPat = Pattern.compile(keyword_expr(gtNegatedList), 
				Pattern.CASE_INSENSITIVE);
