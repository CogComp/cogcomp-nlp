...
public class QueryGenerator {
	...
	public static Predicate<Constituent> generateQuery(Tree<String> tree, Constituent c, Map<String, String> variables) throws
			EdisonException {
	...
	private static Predicate<Constituent> knownQuery(Tree<String> tree, Constituent c, Map<String, String> variables) throws
			EdisonException {

		String label = tree.getLabel();
		if (knownQueries.containsKey(label)) return knownQueries.get(label).create(tree, c, variables);
		else {

			throw new EdisonException(
					"Unknown query '" + label + "'. Expecting one of " + knownQueries.keySet() + ".\n" + tree);
		}

	}

	private static Predicate<Constituent> not(Tree<String> child, Constituent c, Map<String, String> variables) throws
			EdisonException {
		return generateQuery(child, c, variables).negate();
	}

	private static Predicate<Constituent> and(List<Tree<String>> children, Constituent c, Map<String, String> variables) throws
			EdisonException {
	...
	private static Predicate<Constituent> or(List<Tree<String>> children, Constituent c, Map<String, String> variables) throws
			EdisonException {
		Predicate<Constituent> predicate = generateQuery(children.get(0), c, variables);
	...
	private static abstract class KnownQueryHandler {
		...
		Predicate<Constituent> create(Tree<String> tree, Constituent c, Map<String, String> variables) throws
				EdisonException {
		...
		private List<String> getArgs(Tree<String> tree, Map<String, String> variables) throws EdisonException {
			List<String> args = new ArrayList<>();
			if (tree.getNumberOfChildren() != nArgs) throw new EdisonException(
					"Invalid syntax for '" + tree.getLabel() + "'. Expecting " + nArgs + " arguments, found + " + tree
							.getNumberOfChildren() + " instead.\n" + tree);

			for (int i = 0; i < nArgs; i++) {
				String label = tree.getChild(i).getLabel();
				if (variables.containsKey(label)) label = variables.get(label);
				args.add(label);
			}
			return args;
		}
