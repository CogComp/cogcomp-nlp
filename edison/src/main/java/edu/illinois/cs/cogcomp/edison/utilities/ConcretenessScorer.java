...
/**
 * A concreteness scorer based on the ratings of
 * <a href="http://crr.ugent.be/archives/1330">Brysbaert et al. (2014)</a>
 * <p/>
 * <br/>
...
public class ConcretenessScorer {
	...
	private static void readRatingsFile() throws IOException, URISyntaxException {
		...
		InputStream resource = IOUtils.lsResources(LevinVerbClassFeature.class, FILE).get(0).openStream();
		List<String> lines = LineIO.read(resource, Charset.defaultCharset().name(), new ITransformer<String, String>() {
	...
	public static double getRating(String word) throws IOException, URISyntaxException {
		if (ratingMap == null) readRatingsFile();
		if (ratingMap.containsKey(word)) return ratingMap.get(word);
		return 0;
	}
}
