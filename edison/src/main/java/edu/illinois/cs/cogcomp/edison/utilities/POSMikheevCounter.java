 ...
 * length is at least 2 greater than the suffix are extracted. The learned rules
 * are divided into three categories: words that are capitalized and appear
 * first in the sentence, words that are capitalized and don't appear first in
 * the sentence, and uncapitalized, unhyphenated words.
 * 
 * @author Xinbo Wu
 */
public class POSMikheevCounter extends POSBaseLineCounter {
	 ...
	 *
	 * @param table
	 *            The table in which a count should be incremented.
	 * @param suffix
	 *            The suffix.
	 * @param tag
	 *            The POS tag.
	...
	public void prune(HashMap<String, TreeMap<String, Integer>> table) {
		for (Iterator<Map.Entry<String, TreeMap<String, Integer>>> I = table.entrySet().iterator(); I.hasNext();) {
			Map.Entry<String, TreeMap<String, Integer>> e = I.next();
			int sum = 0;
			for (Integer count : e.getValue().values())
				sum += count;

			for (Iterator<Map.Entry<String, Integer>> J = e.getValue().entrySet().iterator(); J.hasNext();) {
