...
public class FeatureUtilities {
	...
	public static edu.illinois.cs.cogcomp.lbjava.classify.Feature getLBJFeature(Feature feature) {
		...
		else return new RealPrimitiveFeature("", "", new ByteString(feature.getName()), feature.getValue());
	}
	 ...
	 * constituent. <b>Note</b>: If there are any real valued features, they
	 * will be ignored.
	 */
	public static Set<String> getFeatureSet(final FeatureExtractor fex, Constituent c) throws EdisonException {
		...
		for (Feature f : fex.getFeatures(c)) {
			if (f instanceof DiscreteFeature) set.add(f.getName());
		}
	 ...
	 * @param trainingMode Should an unseen feature string be added to the lexicon? If
	 *                     this is false, unseen features will be given an ID whose value
	 *                     is one more than the number of features.
	 * @return a pair of int[] and double[], representing the feature ids and
	 * values.
	 */
	...
	/**
	 * This function gets the feature from the lexicon using the id. Use this
	 * function if you use either getFeatureId or convert to avoid off-by-one
	 * errors.
	 */
	...
	/**
	 * Returns the id of the feature according to the lexicon. If trainingMode
	 * is set to false and the feature is not found in the lexicon, then the
	 * function returns -1.
	 */
	public static int getFeatureId(Lexicon lexicon, boolean trainingMode, Feature feature) {
		...
		if (f instanceof RealPrimitiveFeature) f = f.getFeatureKey(lexicon, trainingMode, -1);
		...
		// about it.
		if (!trainingMode) {
			if (id == lexicon.getCutoff()) return -1;
			else {
	...
	/**
	 * Conjoins two feature extractors. This creates a new feature extractor
	 * that produces conjoined features.
	 *
	...
	public static FeatureExtractor conjoin(final FeatureExtractor f1, final FeatureExtractor f2) {
		return new FeatureExtractor() {
			...
			public String getName() {
				if (f1.getName().length() > 0 && f2.getName().length() > 0) return f1.getName() + "&" + f2.getName();
				else if (f1.getName().length() > 0) return f1.getName();
				else if (f2.getName().length() > 0) return f2.getName();
				else return "";
			}
	...
	/**
	 * Conjoins two feature sets. This produces a cross product of features.
	 * That is, every feature from the first parameter is conjoined with every
	 * feature of the second one. If either of the input sets are empty, this
