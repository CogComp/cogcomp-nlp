...
/**
 * This is an extended version of the WordNetLemmaViewGenerator. If the word is
 * a verb, then it first looks up the lemma dictionary. If found, it returns the
 * lemma. Otherwise, it checks against a known list of contractions. Finally, it
 * goes to WordNet. If the word is present in WordNet, the lemma from WordNet is
 * used. Otherwise, it defaults to the word itself (with case).
 * <p/>
 * This lemmatizer lowercases words before finding their lemma. This could lead
 * to errors. This requires the file resources/verb-lemDict.txt in the
 * classpath, which can be obtained from cogcomp-common-resources.
 *
...
public class WordNetPlusLemmaViewGenerator implements Annotator {
	...
	public View getView(TextAnnotation ta) {
		...
		for (int i = 0; i < ta.size(); i++) {
			...
			} else if (POSUtils.isPOSClosedSet(pos)) lemma = word;
			else lemma = ta.getToken(i);
	...
	public static class VerbLemmaDictionary {
		...
		VerbLemmaDictionary(Logger log) throws IOException, URISyntaxException, EdisonException {
			...
			InputStream in = IOUtils.lsResources(VerbLemmaDictionary.class, file).get(0).openStream();
