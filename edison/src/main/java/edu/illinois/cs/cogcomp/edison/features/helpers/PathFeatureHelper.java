...
public class PathFeatureHelper {
	 ...
	 * to the root node. If the path is longer than maxDepth, then it is
	 * truncated.
	 * <p/>
	 * Assumptions:
	 * <ol>
	 * <li>This function follow traces the path from the given node along the
	 * <i>first</i> incoming edge. This is okay if we are dealing with trees.</li>
	 * <li>The "root" node is defined as a node which does not have any incoming
	 * edges. In Edison terms, it is a constituent that does not have any
	 * incoming relations.</li>
	 * </ol>
	 *
	 * @return A list of {@link Constituent}s, where the first element is the
	 * one that is given to the function and the last element is the
	 * root.
	 */
	...
	/**
	 * Get the paths from two constituents to their common ancestor. Each path
	 * is truncated to a length of maxDepth if it is longer than that.
	 * <p/>
	 * <b>Note:</b> This function requires the two constituents to be from the
	 * same {@link View}.
	 *
	 * @throws IllegalArgumentException If no common ancestor is found.
	 * @see edu.illinois.cs.cogcomp.edison.features.helpers.PathFeatureHelper#getPathToRoot(Constituent, int)
	 */
	...
	public static Constituent getCommonAncestor(Constituent start, Constituent end, int maxDepth) {
		Pair<List<Constituent>, List<Constituent>> paths = getPathsToCommonAncestor(start, end, maxDepth);

		return paths.getFirst().get(paths.getFirst().size() - 1);
	}

	/**
	 * Returns the path from start to end as a String. The two boolean
	 * parameters (includeConstituentLabel and includeRelationlabel) specify if
	 * the node and edge labels should be in the string representation. At least
	 * one of them should be true. Othwewise, the function throws an
	...
	public static String getPathString(Constituent start, Constituent end, int maxDepth, boolean includeConstituentLabel, boolean includeRelationLabel) {
		...
		if (!includeConstituentLabel && !includeRelationLabel) throw new IllegalArgumentException(
				"Path string should include at least one of " + "constituent labels and relation labels");
	...
	public static String getPathString(Pair<List<Constituent>, List<Constituent>> paths, boolean includeConstituentLabel, boolean includeRelationLabel) {
		...
		for (int i = 0; i < paths.getFirst().size() - 1; i++) {
			...
			if (includeConstituentLabel) sb.append(c.getLabel());

			if (includeRelationLabel) sb.append(":").append(c.getIncomingRelations().get(0).getRelationName());
		...
		if (includeConstituentLabel) sb.append(top.getLabel());

		if (includeRelationLabel) sb.append("ROOT");
	...
	/**
	 * Get the dependency path between two Constituents as a String. Both the
	 * start and end constituents should be in a dependency view.
	 */
	public static String getDependencyPathString(Constituent startDependencyConstituent, Constituent endDependencyCOnstituent, int maxDepth) {
		...
		return getPathString(startDependencyConstituent, endDependencyCOnstituent, maxDepth, false, true);
	}
