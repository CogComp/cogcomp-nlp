...
public class ParseHelper {
	...
	public static String[] getAllSiblingLabels(Tree<String> node) {
		...
		if (node.isRoot()) return new String[0];
	 ...
	 */
	@Deprecated
	public static <T> Tree<T> getCommonAncestor(Tree<T> t1, Tree<T> t2, Tree<T> tree) throws Exception {
	...
	public static Tree<String> getParseTree(String parseViewName, Sentence s) {
		return getParseTree(parseViewName, s.getSentenceConstituent().getTextAnnotation(), s.getSentenceId());
	}
	 ...
	 */
	@Deprecated
	public static <T> Pair<List<Tree<T>>, List<Tree<T>>> getPath(Tree<T> start, Tree<T> end, Tree<T> tree, int maxDepth) throws
			Exception {
		...
		if (!foundAncestor) throw new Exception("Common ancestor not found in path down.");
		...
		if (!foundAncestor) throw new Exception("Common ancestor not found in path up.");
	 ...
	 */
	@Deprecated
	public static <T> String getPathString(Tree<T> start, Tree<T> end, Tree<T> tree, int maxDepth) throws Exception {
	...
	@Deprecated
	public static <T> String getPathStringIgnoreLexicalItems(Tree<T> start, Tree<T> end, Tree<T> tree, int maxDepth) throws
			Exception {
	...
	@Deprecated
	public static <T> String getPathStringToCommonAncestor(Tree<T> start, Tree<T> end, Tree<T> tree, int maxDepth) throws
			Exception {
		Pair<List<Tree<T>>, List<Tree<T>>> paths = getPath(start, end, tree, maxDepth);
	...
	@Deprecated
	public static <T> List<T> getPathToRoot(Tree<T> tree, Tree<T> leaf, int maxDepth) throws Exception {
	 ...
	 */
	@Deprecated
	public static <T> List<Tree<T>> getPathTreesToRoot(Tree<T> tree, Tree<T> node, int maxDepth) throws Exception {
		...
		while (!t.equals(tree)) {
			...
			if (t == null) throw new Exception(node + " is not an element of " + tree);
			depth++;
			if (depth > maxDepth) {
				break;
			}
		...
		ITransformer<Tree<Pair<String, IntPair>>, Pair<String, IntPair>> transformer =
				new ITransformer<Tree<Pair<String, IntPair>>, Pair<String, IntPair>>() {
					...
					public Pair<String, IntPair> transform(Tree<Pair<String, IntPair>> input) {
						...
						IntPair newSpan = new IntPair(label.getSecond().getFirst() + sentenceStartSpan,
								label.getSecond().getSecond() + sentenceStartSpan);
		...
		while (current != null) {
			...
			if (span.getFirst() == start && span.getSecond() == end) {
				...
				for (Tree<Pair<String, IntPair>> child : current.getChildren()) {
					if (child.getLabel().getSecond().getFirst() <= start && child.getLabel().getSecond()
							.getSecond() >= end) {
						current = child;
						found = true;
						break;
	 ...
	 * @return The index of the head word. The word itself can be obtained later
	 * by calling {@link WordHelpers#getWord(TextAnnotation, int)}.
	 * @throws EdisonException
	 */
	public static int getHeadWordPosition(Constituent c, HeadFinderBase headFinder, String parseViewName) throws
			Exception {
		TextAnnotation ta = c.getTextAnnotation();
	...
	public static Constituent getPhraseFromHead(Constituent predicate, Constituent argHead, String parseViewName) {
		...
		for (Tree<Pair<String, IntPair>> s : predParentTree.getYield()) {
			if (s.getLabel().getSecond().getFirst() == argStart) found = true;
		}
