...
public class VerbClassDictionary {
	...
	/**
	 * Load the verb class dictionary from the file. The file consists of
	 * Levin's verb classes and the verbs they contain in the following format:
	 * <p/>
	 * <pre>
	 *  13.4.1
	...
	public VerbClassDictionary(InputStream inputStream) throws IOException {
		...
		String verbClass = "";
		String line;
		while ((line = in.readLine()) != null) verbClass = readVerbClasses(verbClass, line);
		...
		if (verbClassDictionary == null) {
			synchronized (LevinVerbClassFeature.class) {
				...
				if (verbClassDictionary == null) {
					...
					log.info("Reading verb class dictionary. Looking for " + verbClassFile + " in the classpath");
					try {
	...
	private String readVerbClasses(String verbClass, String line) {
		if (line.startsWith(" ")) {
			...
			for (String verb : line.split(" +")) {
				...
				if (!verbClasses.containsKey(verb)) verbClasses.put(verb, new ArrayList<String>());
	...
	public List<String> getClass(String verb) {
		...
		if (verbClasses.containsKey(verb)) return verbClasses.get(verb);
