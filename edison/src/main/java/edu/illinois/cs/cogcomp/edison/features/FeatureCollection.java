...
/**
 * A feature set. This feature extractor can chain several other feature
 * extractors.
 *
...
public class FeatureCollection implements FeatureExtractor {
	...
	/**
	 * Create a new feature extractor. By itself, it does not generate any
	 * features. Use
	 * {@code FeatureExtractor#addFeatureGenerator(IFeatureExtractor))} to add
	 * feature generators.
	 */
	...
	public FeatureCollection(String name, FeatureInputTransformer inputTransformer, FeatureExtractor... feats) {
		this.name = name;
	...
	private void getFeatures(boolean hasName, Set<Feature> features, Constituent in) throws EdisonException {
		for (FeatureExtractor f : generators) {
			...
			for (Feature feat : feats)
				...
				if (hasName && fHasName) features.add(feat.prefixWith(this.getName() + ":" + name));
				else if (!hasName && fHasName) features.add(feat.prefixWith(name));
				else if (hasName && !fHasName) features.add(feat.prefixWith(getName()));
				else features.add(feat);
		}
	}
