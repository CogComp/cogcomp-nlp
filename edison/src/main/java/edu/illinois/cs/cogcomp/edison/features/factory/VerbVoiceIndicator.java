 ...
 * voice. If the word is not a verb or not in a VP, then the word is marked as
 * having "unknown" voice.
 * <p/>
 * A predicate is either active or passive. We use a simple rule to determine
 * this: if the verb follows a "to-be" verb in the same verb phrase and its POS
 * tag is "VBN", then the predicate is passive. The verb may also be nested in
 * other verb phrases that are themselves nested in the verb phrase containing
 * the "to-be" verb, so long as the only phrases found in between the "to-be"
 * verb and the target verb contain the target verb.
 *
...
public class VerbVoiceIndicator extends WordFeatureExtractor {
	...
	private String getVoice(Tree<Pair<String, IntPair>> currentNode) {
		...
		if (!POSUtils.isPOSVerb(pos)) return UNKNOWN;
			...
			while (parent != null && parent.getLabel().getFirst().equals("VP")) {
				...
				for (int i = children.size() - 1; i >= 0; --i) {
					if (children.elementAt(i).getChild(0).isLeaf()) {
						if (WordLists.toBeVerbs.contains(children.elementAt(i).getChild(0).getLabel().getFirst()))
							return PASSIVE;
					}
				}
				sentinel = parent;
