 ...
 * constituent is in a list or not. If it is present, then it emits the name of
 * the list as a feature.
 *
...
public class ListFeatureExtractor implements FeatureExtractor {
	 ...
	 * @param list          The list of items against which the constituent should be
	 *                      checked.
	...
	public ListFeatureExtractor(String name, Collection<String> list, boolean caseSensitive) {
		...
		for (String l : list)
			if (caseSensitive) this.list.add(l);
			else this.list.add(l.toLowerCase());
	 ...
	 */
	public ListFeatureExtractor(String listName, String listFile, boolean caseSensitive) throws FileNotFoundException {
	...
	public Set<Feature> getFeatures(Constituent c) throws EdisonException {
		...
		if (!caseSensitive) surface = surface.toLowerCase();
		Set<Feature> features = new LinkedHashSet<>();
		if (list.contains(surface)) features.add(listName);
		return features;
	}
