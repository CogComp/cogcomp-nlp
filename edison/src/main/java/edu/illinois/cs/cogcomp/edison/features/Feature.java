	...
	public Feature conjoinWith(Feature f) {
		...
		if (ascii && this.bytes != null && f.bytes != null) {
			...
			if (f instanceof DiscreteFeature) {
				if (this instanceof DiscreteFeature) return new DiscreteFeature(newBytes);
				else return new RealFeature(newBytes, this.getValue());
			} else {
				if (this instanceof DiscreteFeature) return new RealFeature(newBytes, f.getValue());
				else return new RealFeature(newBytes, f.getValue() * this.getValue());
			...
			if (f instanceof DiscreteFeature) {
				if (this instanceof DiscreteFeature) return new DiscreteFeature(newName);
				else return new RealFeature(newName, this.getValue());
			} else {
				if (this instanceof DiscreteFeature) return new RealFeature(newName, f.getValue());
				else return new RealFeature(newName, f.getValue() * this.getValue());
	...
	public Feature prefixWith(String s) {
		...
		if (ascii && this.bytes != null) {
			...
			if (keepString) {
				...
				if (this instanceof DiscreteFeature) return new DiscreteFeature(name);
				else return new RealFeature(name, this.getValue());
