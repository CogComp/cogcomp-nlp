 ...
 * speech views. The implementation assumes that the input text annotation
 * contains those views.
 *
...
public class PseudoParse extends Annotator {
	...
	public static final PseudoParse BERKELEY =
			new PseudoParse(ViewNames.CLAUSES_BERKELEY, ViewNames.PSEUDO_PARSE_BERKELEY);
	public static final PseudoParse CHARNIAK =
			new PseudoParse(ViewNames.CLAUSES_CHARNIAK, ViewNames.PSEUDO_PARSE_CHARNIAK);
	...
	public void addView(TextAnnotation ta) {
		...
		for (Constituent c : clauses) {
			if (endOfChunks.contains(c.getStartSpan())) {
				...
				output.add(new Pair<>(c.getStartSpan() - 0.3,
						"(" + c.getLabel() + "-" + shallowParse.get(index).getLabel()));
			} else output.add(new Pair<>(c.getStartSpan() - 0.3, "(" + c.getLabel()));
