...
public class NomLexReader {
	...
	public synchronized static NomLexReader getInstance() throws EdisonException {
		if (!initialized) {
			if (nomLexFile == null) {
				log.error(
						"Cannot initialize NomLex." + "Please set NomLexReader.nomLexFile " + "to the correct value.");
	...
	private void readNomLexLisp(BufferedReader in) throws IOException {
		...
		while ((line = in.readLine()) != null) {
			...
			for (int i = 0; i < line.length(); i++) {
				...
				if (c == '"') {
					if (i > 0 && line.charAt(i - 1) != '\\') inQuotes = !inQuotes;
				} else if (c == '(') {
					if (!inQuotes) {
						numParens++;
					} else c1 = "[";
				...
				if (prevChar.equals(c1) && c1.equals("(")) current.append("DummyNode");
			...
			if (numParens == 0) {
				...
				if (!this.nomLex.containsKey(entry.orth)) this.nomLex.put(entry.orth, new ArrayList<NomLexEntry>());
	...
	private NomLexEntry createNewRecord(String string) {
		...
		for (int childId = 0; childId < tree.getNumberOfChildren(); childId++) {
			...
			Tree<String> value;
			if (key.isLeaf()) {
				if (childId == tree.getNumberOfChildren() - 1) break;
				value = tree.getChild(childId + 1);
			} else value = key.getChild(0);
			...
			switch (label) {
				...
				case ":ADJ":
					...
					break;
				case ":PLURAL":
					if (!valueLabel.equals("*NONE*")) entry.plural = valueLabel;
					else entry.plural = null;
					break;
			}
		}

		if (entry.plural == null) entry.plural = entry.orth;
		else pluralToSingularMap.put(entry.plural, entry.orth);
	...
	public List<NomLexEntry> getNomLexEntry(String token) {
		if (this.nomLex.containsKey(token)) return this.nomLex.get(token);
	...
	public List<NomLexEntry> getNomLexEntries(String predicateWord, String predicateLemma) {
		...
		if (this.containsEntry(predicateWord)) return this.getNomLexEntry(predicateWord);
		else if (this.containsEntry(predicateLemma)) return this.getNomLexEntry(predicateLemma);
