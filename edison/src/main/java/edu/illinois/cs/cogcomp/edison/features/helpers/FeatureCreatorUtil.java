...
public class FeatureCreatorUtil {
    ...
    /**
     * adjust initial window start/end based on how far word context extends in terms of the underlying text/sentence
     *    (depending on ignoreSentenceBoundaries flag).
     * <b>IMPORTANT:</b> this expresses a window relative to the Constituent start and end token indexes, so the end
     *    will be different for a 3-token NER Constituent than for a Word Constituent.
     ...
     * @param ignoreSentenceBoundaries if 'true', allow window to extend past a sentence boundary iff there are
     *                                 tokens at the relevant indexes.
    ...
    public static IntPair getWindowSpan(Constituent c, int windowStart, int windowEnd, boolean ignoreSentenceBoundaries) {
        ...
        if ( !ignoreSentenceBoundaries ) {
            List<Constituent> sentences = c.getTextAnnotation().getView(ViewNames.SENTENCE).getConstituentsCovering(c);
