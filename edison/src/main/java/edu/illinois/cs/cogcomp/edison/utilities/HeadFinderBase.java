...
public abstract class HeadFinderBase {
	...
	protected Pair<HeadSearchDirection, String[]> defaultRule = new Pair<>(HeadSearchDirection.Left, new String[]{"S"});
	...
	public Constituent getHeadChild(Constituent parseNode) {
		...
		if (predefinedHead != null) return predefinedHead;

		// if it is the leaf, then return it.
		if (parseNode.getOutgoingRelations().size() == 0) return parseNode;

		// if only one child then easy
		if (parseNode.getOutgoingRelations().size() == 1) return parseNode.getOutgoingRelations().get(0).getTarget();
	...
	protected Constituent findHead(Constituent parseNode) {
		...
		if (headInfo == null) {
			...
			else return findHead(parseNode, defaultRule, true);
		}
	 ...
	 * find the head.
	 */
	private Constituent findHead(Constituent parseNode, Pair<HeadSearchDirection, String[]> rule, boolean getDefaultHeadChild) {

		int headChild = rule.getFirst().getHeadChildId(parseNode, rule.getSecond(), getDefaultHeadChild);

		if (headChild == -1) return null;
	 ...
	 */
	public abstract List<Pair<HeadSearchDirection, String[]>> getNonterminalHeadInformation(String nonTerminal);
	...
	public enum HeadSearchDirection {
		...
		Right {
			public int getHeadChildId(Constituent tree, String[] info, boolean getDefault) {
				...
				// traverse from left to right
				for (String anInfo : info) {
					for (int headCandidate =
						 tree.getOutgoingRelations().size() - 1; headCandidate >= 0; headCandidate--) {
				...
				if (!foundHead) {
					if (getDefault) headId = tree.getOutgoingRelations().size() - 1;
					else headId = -1;
				}
