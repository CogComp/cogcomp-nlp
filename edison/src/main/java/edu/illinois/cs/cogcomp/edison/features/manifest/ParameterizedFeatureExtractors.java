...
class ParameterizedFeatureExtractors {
	...
	private static final ParameterizedFeatureGenerator voiceGenerator = new ParameterizedFeatureGenerator(PARSE_VIEW) {

		@Override
		public FeatureExtractor getFeatureExtractor(Map<String, String> attributes, Tree<String> tree, FeatureExtractor fex) throws
				EdisonException {
			return new VerbVoiceIndicator(attributes.get(PARSE_VIEW));
		}
	};
	private static final ParameterizedFeatureGenerator clauseGenerator = new ParameterizedFeatureGenerator(PARSE_VIEW) {
		...
		public FeatureExtractor getFeatureExtractor(Map<String, String> attributes, Tree<String> tree, FeatureExtractor fex) throws
			...
			if (parse.equals(ViewNames.PARSE_CHARNIAK)) return ClauseFeatureExtractor.CHARNIAK;
			else if (parse.equals(ViewNames.PARSE_BERKELEY)) return ClauseFeatureExtractor.BERKELEY;
			else if (parse.equals(ViewNames.PARSE_STANFORD)) return ClauseFeatureExtractor.STANFORD;
			else throw new EdisonException("Cannot generate clause view for using " + parse);

		}
	};
	private static final ParameterizedFeatureGenerator attributeGenerator = new ParameterizedFeatureGenerator(":name") {
	...
	private static final ParameterizedFeatureGenerator subcatGenerator = new ParameterizedFeatureGenerator(PARSE_VIEW) {
	...
	private static final ParameterizedFeatureGenerator parsePath = new ParameterizedFeatureGenerator(PARSE_VIEW) {
	...
	private static final ParameterizedFeatureGenerator syntacticFrame = new ParameterizedFeatureGenerator(PARSE_VIEW) {
	...
	private static final ParameterizedFeatureGenerator headFeatures = new ParameterizedFeatureGenerator(PARSE_VIEW) {
		...
		public FeatureExtractor getFeatureExtractor(Map<String, String> attributes, Tree<String> tree, FeatureExtractor fex) throws
			...
			if (parseView.equals(ViewNames.PARSE_GOLD) || parseView.equals(ViewNames.PARSE_STANFORD))
				return new ParseHeadWordFeatureExtractor(parseView, fex);

			FeatureInputTransformer transformer;
			if (parseView.equals(ViewNames.PARSE_CHARNIAK)) transformer = FeatureInputTransformer.charniakHead;
			...
			else if (parseView.equals(ViewNames.PARSE_BERKELEY)) transformer = FeatureInputTransformer.berkeleyHead;
			else throw new EdisonException("Invalid parse view: " + parseView + "\n" + tree);
	...
	private static final ParameterizedFeatureGenerator objectFeatures = new ParameterizedFeatureGenerator(PARSE_VIEW) {
		...
		public FeatureExtractor getFeatureExtractor(Map<String, String> attributes, Tree<String> tree, FeatureExtractor fex) throws
			...
			FeatureInputTransformer transformer;
			if (parseView.equals(ViewNames.PARSE_CHARNIAK)) transformer = FeatureInputTransformer.charniakObject;
			else if (parseView.equals(ViewNames.PARSE_STANFORD)) transformer = FeatureInputTransformer.stanfordObject;
			else if (parseView.equals(ViewNames.PARSE_BERKELEY)) transformer = FeatureInputTransformer.berkeleyObject;
			else if (parseView.equals(ViewNames.DEPENDENCY_STANFORD))
	...
	private static final ParameterizedFeatureGenerator context =
			new ParameterizedFeatureGenerator(":size", ":include-index", ":ignore-center") {

				@Override
				FeatureExtractor getFeatureExtractor(Map<String, String> attributes, Tree<String> tree, FeatureExtractor fex) throws
						EdisonException {
					...
					ContextFeatureExtractor f = new ContextFeatureExtractor(size, specifyIndex, ignoreConstituent);
	...
	private static ParameterizedFeatureGenerator dependencyModifiers = new ParameterizedFeatureGenerator(PARSE_VIEW) {
	...
	private static ParameterizedFeatureGenerator parseSiblings = new ParameterizedFeatureGenerator(PARSE_VIEW) {
	...
	private static ParameterizedFeatureGenerator regexFeatureGenerator = new ParameterizedFeatureGenerator(":regex") {
	...
	static FeatureExtractor getParameterizedFeatureExtractor(Tree<String> tree, FeatureExtractor fex, HashMap<String, String> variables) throws
			EdisonException {
	...
	private static abstract class ParameterizedFeatureGenerator {
		...
		FeatureExtractor getFeatureExtractor(Tree<String> tree, FeatureExtractor fex, HashMap<String, String> variables) throws
				EdisonException {
		...
		abstract FeatureExtractor getFeatureExtractor(Map<String, String> attributes, Tree<String> tree, FeatureExtractor fex) throws
				EdisonException;

		private void checkPrerequisites(Map<String, String> attributes, Tree<String> tree, String name) throws
				EdisonException {
			if (!attributes.containsKey(name))
				throw new EdisonException("Required parameter" + name + " missing\n" + tree);
		}

		private Map<String, String> getAttributes(Tree<String> tree, HashMap<String, String> variables) throws
				EdisonException {
			...
			while (childId < tree.getNumberOfChildren()) {
				...
				if (label.startsWith(":")) {
					...
					if (!child.isLeaf()) throw new EdisonException("Parameter " + label + " should be a leaf\n" + tree);
					...
					if (variables.containsKey(value)) value = variables.get(value);
