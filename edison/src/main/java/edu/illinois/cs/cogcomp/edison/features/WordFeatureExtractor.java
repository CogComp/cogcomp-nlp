 ...
 * it will throw a {@link EdisonException}.
 *
...
public abstract class WordFeatureExtractor implements FeatureExtractor {
	...
	/**
	 * Creates a new WordFeatureExtractor. If parameter
	 * {@code useLastWordOfMultiwordConstituents} is {@code true}, then the
	 * feature extractor will generate features from the last word of multi-word
	 * constituents. If it is not true, then the feature extractor will throw an
	 * exception on seeing a multi-word constituent.
	 * <p/>
	 * It is probably safest to the parameter to {@code true}. This will provide
	 * a check to ensure that the WordFeatureExtractor only sees words.
	 */
	...
	/**
	 * Creates a new {@link edu.illinois.cs.cogcomp.edison.features.WordFeatureExtractor}. This constructor is
	 * equivalent to calling {@code new WordFeatureExtractor(true)}.
	 *
	...
	public Set<Feature> getFeatures(Constituent c) throws EdisonException {
		...
		if (c.size() != 1) {
			if (useLastWordOfMultiwordConstituents) wordPosition = c.getEndSpan() - 1;
			else throw new EdisonException(
					"Input \"" + c.getTokenizedSurfaceForm() + "\" to WordFeatureExtractor is not a single word. ");
		} else wordPosition = c.getStartSpan();

		return getWordFeatures(c.getTextAnnotation(), wordPosition);

	}

	public abstract Set<Feature> getWordFeatures(TextAnnotation ta, final int wordPosition) throws EdisonException;
