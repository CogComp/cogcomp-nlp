...
public class TestWordFeatures extends TestCase {
	
	private static List<TextAnnotation> tas;

	static {
		try {
			tas = IOUtils.readObjectAsResource(WordFeatures.class, "test.ta");
		} catch (Exception e) {
	...
	public final void test() throws Exception {
		System.out.println("WordFeatures Feature Extractor");
...
//		System.out.println("Testlist size is " + testlist.size());
		...
		String fileName = Constant.prefix + "\\Verb\\Sense\\word-features.fex";
...
//		}
		...
		WordFeatures wf = new WordFeatures(); 
	
//		featslist.clear();
//
//		for (Constituent test : testlist)
//			featslist.add(wf.getFeatures(test));
//
//		if (featslist.isEmpty()) {
//			System.out.println("Feats list is returning NULL.");
//		}
...
//		}
		...
		for (Constituent test : testlist){
			System.out.println("The constituent for testing is " + test.toString());
			Set<Feature> feats = wf.getFeatures(test);
			for (Feature f : feats){
				System.out.println(f.getName());
				EdisonFexCount += f.getName().split("/n").length;
			}
