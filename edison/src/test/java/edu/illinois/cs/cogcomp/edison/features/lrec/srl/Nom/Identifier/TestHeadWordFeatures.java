...
import edu.illinois.cs.cogcomp.edison.features.lrec.srl.Nom.Identifier.PredicateFeatures;
...
import edu.illinois.cs.cogcomp.edison.features.lrec.srl.Nom.Identifier.WordContext;
...
public class TestHeadWordFeatures extends TestCase {
	
	private static List<TextAnnotation> tas;

	static {
		try {
			tas = IOUtils.readObjectAsResource(HeadWordFeatures.class, "test.ta");
		} catch (Exception e) {
	...
	public final void test() throws Exception {
		System.out.println("HeadWordFeatures Feature Extractor");
...
//		}
		...
		HeadWordFeatures hwf = new HeadWordFeatures(); 
	
//		featslist.clear();
//
//		for (Constituent test : testlist)
//			featslist.add(hwf.getFeatures(test));
//
//		if (featslist.isEmpty()) {
//			System.out.println("Feats list is returning NULL.");
//		}
...
//		}
		...
		for (Constituent test : testlist){
			System.out.println("The constituent for testing is " + test.toString());
			Set<Feature> feats = hwf.getFeatures(test);
			for (Feature f : feats){
				System.out.println(f.getName());
				SRLFexCount += f.getName().split("/n").length;
			}
