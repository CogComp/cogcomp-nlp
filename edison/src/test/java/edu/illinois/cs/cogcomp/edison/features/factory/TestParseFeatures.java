...
public class TestParseFeatures extends TestCase {
	...
	public final void testVerbVoiceIndicator() throws Exception {
		...
		for (TextAnnotation ta : tas) {
			if (!ta.hasView(ViewNames.SRL_VERB)) continue;
			...
			for (Constituent predicate : pav.getPredicates()) {
				System.out.println(predicate + "\t" + VerbVoiceIndicator.CHARNIAK.getFeatures(predicate));
			}
		}
		...
		for (TextAnnotation ta : tas) {
			...
			System.out.println(ta);
			for (Constituent predicate : pav.getPredicates()) {
				System.out.println(predicate + "\t" + VerbVoiceIndicator.STANFORD.getFeatures(predicate));
			}
		}
