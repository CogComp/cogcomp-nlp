    ...
    static public void computeCharacterOffsets(ArrayList<OntoNotesDataModel> sentences) {
        ...
        for (int i = 0; i < sentences.size(); i++) {
            sentence_start_offset =
                    computeCharacterOffsetsForEachSentence(sentences, i, sentence_start_offset);
            sentences.get(i).setSentenceStartOffset(sentences.get(i).getStartOffsets().get(0));
            sentences.get(i).setSentenceEndOffset(
                    sentences.get(i).getEndOffsets()
                            .get(sentences.get(i).getEndOffsets().size() - 1));
        }
    }

    static private int computeCharacterOffsetsForEachSentence(
            ArrayList<OntoNotesDataModel> sentences, int index, int sentence_start_offset) {
