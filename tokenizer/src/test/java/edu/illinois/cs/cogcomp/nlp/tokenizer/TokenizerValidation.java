...
/**
 * Using an instance of a MySQL database containing the ontonotes data,
 * validate the tokenizers against one another. The database contains
 * the full sentences, and the tokens extracted by the OntoNotes project,
 * this class will run our tokenizers (Stateful and Illinois) and see how
 * they compare, reporting any differences.
 * @author redman
 *
 */
public class TokenizerValidation {
    ...
    private static void parseArgs(String [] args) {
    	if (args.length != 3) {
    		System.err.println("You must pass <user account>, <password>, and <database URL> as arguments to this function.");
    		System.exit(-1);
    	}
    	account = args[0];
    	password = args[1];
	...
	public static void main(String[] args) throws SQLException {
	    ...
	    try (ResultSet rs1 = con.createStatement().executeQuery(sentencequery)) {
	    	while (rs1.next()) {
	            ...
	            /**
	            String tokequery ="select t.word from token t where t.id like '%"+id+"' AND part_of_speech!='-NONE-'";
	            try (ResultSet rs2 = con.createStatement().executeQuery(tokequery)) {
	            	int iindex = 0;
	            	int sindex = 0;
	            	while (rs2.next()) {
	            		String word = rs2.getString(1);
	            		if (sindex >= statefulToks.size()) {
	            			System.out.println("On token '"+word+", stateful parsing revealed too few tokens.");
	            		} else {
	            			String stok = statefulToks.get(sindex).getSurfaceForm();
	            			if (!word.equals(stok))
		            			System.out.println("On token '"+word+"', stateful parsing token was '"+stok+"'");
	            		}
	            		if (iindex >= ilToks.size()) {
	            			System.out.println("On token '"+word+"', illinois parsing revealed too few tokens.");
	            		} else {
	            			String stok = ilToks.get(iindex).getSurfaceForm();
	            			if (!word.equals(stok))
