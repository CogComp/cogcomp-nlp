...
/**
 * This class defines the behavior of an annotation service that can
 * be fulfilled by an arbitrary backend, and which will cache the annotations so
 * fulfilled. Sentence and Token views are populated by the TextAnnotationBuilder object
 * passed to the constructor.  By default, only Annotators provided as 'viewProviders'
 * will be applied to input text. Their dependencies will be (minimally) checked to make sure a requested
 * view has its required inputs provided.
 * By default, all annotation views are cached.  The constructor provides default values for caching behavior
 * (see the default values), but these can be overridden by providing a ResourceManager (and configuration
 * file) that specifies these values.
 * <p/>
 * Many Annotators are likely to be singletons to avoid memory overload. AnnotatorService can be subclassed to
 * provide homespun views without creating Annotator classes, though probably the latter is the better way to go.
 * There is at least one use case that motivates this design -- backward compatibility with Curator to replace
 * CuratorClient/CachingCuratorClient.
 * <p/>
 *
 * @author Mark Sammons
...
public class BasicAnnotatorService implements AnnotatorService
{
    ...
    /**
     * used when processing a corpus and then using it in experiments --
     * if 'true', you will know in the experimental run that a data element is missing.
     */
    protected boolean throwExceptionIfNotCached = false;

    /**
     * provides tokenization and sentence splitting to create a basic TextAnnotation object
     * from plain text String.
     */
    protected TextAnnotationBuilder textAnnotationBuilder;

    /**
     * A way to define externally provided views. Each view provider needs to
     * implement {@link Annotator}.
     */
    ...
    public BasicAnnotatorService(TextAnnotationBuilder textAnnotationBuilder,
                                 ...
                                 ResourceManager rm) throws AnnotatorException {
        this(textAnnotationBuilder, viewProviders,
                rm.getString(AnnotatorServiceConfigurator.CACHE_DIR.key),
                rm.getBoolean(AnnotatorServiceConfigurator.THROW_EXCEPTION_IF_NOT_CACHED.key),
                rm.getInt(AnnotatorServiceConfigurator.CACHE_HEAP_SIZE_MB.key),
                rm.getInt(AnnotatorServiceConfigurator.CACHE_DISK_SIZE_MB.key),
                rm.getBoolean(AnnotatorServiceConfigurator.SET_CACHE_SHUTDOWN_HOOK.key),
                rm.getBoolean(AnnotatorServiceConfigurator.DISABLE_CACHE.key),
                rm.getBoolean(AnnotatorServiceConfigurator.FORCE_CACHE_UPDATE.key));
    }
    ...
    public BasicAnnotatorService(TextAnnotationBuilder textAnnotationBuilder,
        ...
        this(textAnnotationBuilder, viewProviders, ( new AnnotatorServiceConfigurator() ).getDefaultConfig() );
    ...
    /**
     * Populates the AnnotatorService with {@link View} providers and sets the {@link Cache} properties.
     *
     * @param textAnnotationBuilder     tokenizes and sentence splits input text.
     * @param viewProviders             Annotators that populate a View with the same name as the corresponding Annotator key.
     * @param cacheDir                  Where the AnnotatorService should write the cache.
     *                                  <b>IMPORTANT</b>: multiple VMs may <b>NOT</b> share a single cache location.
    ...
    public BasicAnnotatorService(TextAnnotationBuilder textAnnotationBuilder,
                                 ...
                                 int cacheHeapSizeInMegabytes,
                                 int cacheDiskSizeInMegabytes,
                                 boolean setCacheShutdownHook,
            ...
            if (null == cacheManager) {
                ...
                if (cacheManager.cacheExists(cName)) {
                    ...
                    CacheConfiguration cacheConfiguration =
                            new CacheConfiguration().eternal(true).name(cName).diskPersistent(true).overflowToDisk(true);
     ...
     * <p/>
     * Also, caches the created {@link TextAnnotation} using the {@link #getTextAnnotationCacheKey(String, String)}
     */
    @Override
    public TextAnnotation createBasicTextAnnotation(String corpusId, String docId, String text ) throws AnnotatorException {
        ...
        if (!disableCache && annotationCache.isKeyInCache(key)) {
            ...
            return SerializationHelper.deserializeTextAnnotationFromBytes((byte[]) taElem.getObjectValue());
        }
    ...
    @Override
    public TextAnnotation createAnnotatedTextAnnotation(String corpusId, String textId, String text, Set<String> viewNames)
            throws AnnotatorException {
        long startTime = System.currentTimeMillis();
        logger.debug( "starting createAnnotatedTextAnnotation()..." );
        TextAnnotation ta = createBasicTextAnnotation(corpusId, text, text );
        for (String view : viewNames) addView(ta, view );
        long endTime = System.currentTimeMillis();
        long duration = (endTime - startTime);
        logger.debug("Finished createAnnotatedTextAnnotation(), took: " + duration + " milliseconds");
    ...
    public boolean addView(TextAnnotation ta, String viewName) throws AnnotatorException {
        if (throwExceptionIfNotCached && !annotationCache.isKeyInCache(getCacheKey(ta, viewName)))
            throw new AnnotatorException("No cache entry for TextAnnotation, and throwExceptionIfNotCached is 'true'. " +
            " Text is: '" + ta.getText() );
        ...
        if (!forceUpdate) {
            if (ta.hasView(viewName))
                ...
                if (annotationCache.isKeyInCache(key)) {
                    ...
                    long endTime = System.currentTimeMillis();
                    logger.debug(PrintUtils.printTimeTakenMs("Finished addView(" + viewName + ") with cache hit.", startTime, endTime));
                    isUpdateNeeded = false;
                }
            }
        }
        ...
        if (isUpdateNeeded) {
            /**
             * at this point, we have to run the annotator b.c. view is not in cache, or cache is disabled,
             *   or client forces update
             */
            addViewAndCache(ta, viewName );

            long endTime = System.currentTimeMillis();

            logger.debug(PrintUtils.printTimeTakenMs("Finished addView(" + viewName + ") with cache miss.", startTime, endTime));
        }
    ...
    /**
     * Generates a hash key based on the basic {@link TextAnnotation} information
     *   (TextAnnotation's own hash function) and the {@link View} name.
     *
