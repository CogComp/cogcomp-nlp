...
public class BordaCount {
    ...
    /**
     *
     * @param rankings A list of rankings of object T.
     *                 It is assumed that the type <code>T</code> implements the <code>hashCode()</code> and
        ...
        if (rankings != null && rankings.size()>0) {
            ...
            for (List<T> ranking: rankings) {
                if (ranking.size() != size)
                    throw new IllegalArgumentException("Number of element in each ranking should be same.");
            }
