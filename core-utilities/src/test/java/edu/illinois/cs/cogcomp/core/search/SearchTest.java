...
public class SearchTest extends TestCase {
    ...
    IStateManager<Pair<Integer, Integer>> stateManager = new IStateManager<Pair<Integer, Integer>>() {

        public boolean goalTest(Pair<Integer, Integer> data) {
            return (data.getFirst() + data.getSecond()) >= 4;
        }

        public List<Pair<Integer, Integer>> nextStates(
                Pair<Integer, Integer> currentState) {

            List<Pair<Integer, Integer>> list = new ArrayList<>();

            list.add(new Pair<>(currentState.getFirst() + 1,
                    currentState.getSecond()));
            list.add(new Pair<>(currentState.getFirst(),
                    currentState.getSecond() + 1));
            return list;
        }
    };
    ...
    public final void testDepthFirstSearch() {
        ...
        Pair<Integer, Integer> result = (dfs.search(new Pair<>(
                0, 0), stateManager));
    ...
    public final void testUniformCostSearch() {
        GraphSearch<Pair<Integer, Integer>> dfs = new UniformCostSearch<>(
    ...
    public final void testBeamSearch() {
        GraphSearch<Pair<Integer, Integer>> dfs = new BeamSearch<>(
                4, new Comparator<Pair<Integer, Integer>>() {
            public int compare(Pair<Integer, Integer> o1,
                               Pair<Integer, Integer> o2) {
                return -score(o1).compareTo(score(o2));
            }
